...
function clearModuleCache() {
  for (const key of Object.keys(require.cache)) {
    if (key.includes('ssb-tennis-context')) {
...
// and lib/ssb-tennis.js before loading the context module under test.
...
describe('module exports', () => {
  it('exports getTennisContext, guessSurfaceFromTournament, guessMatchLevel', () => {
    const mod = require('../lib/ssb-tennis-context');
...
describe('guessSurfaceFromTournament', () => {
  ...
  it('returns null for empty / non-string input', () => {
    const { guessSurfaceFromTournament } = require('../lib/ssb-tennis-context');
...
describe('guessMatchLevel', () => {
  ...
  it('returns null for empty / non-string input', () => {
    const { guessMatchLevel } = require('../lib/ssb-tennis-context');
...
describe('getTennisContext', () => {
  ...
  it('returns unknown surface and null level for empty params', async () => {
    const { getTennisContext } = require('../lib/ssb-tennis-context');
...
describe('looksLikeMatchup', () => {
  ...
  it('returns true for "Dart vs Sonmez"', () => {
    const { looksLikeMatchup } = require('../lib/ssb-tennis-context');
...
describe('parseMatchup', () => {
  ...
  it('splits " vs " into two players', () => {
    const { parseMatchup } = require('../lib/ssb-tennis-context');
...
describe('resolveTournamentFromMatchup', () => {
  ...
  it('resolves Dart vs Sonmez (2026-06-22) to Eastbourne (WTA grass)', () => {
    const { resolveTournamentFromMatchup } = require('../lib/ssb-tennis-context');
...
describe('PLAYER_CIRCUIT coverage (regression)', () => {
  ...
  for (const tc of KNOWN_MATCHUPS) {
    it(`resolves ${tc.matchup} @ ${tc.start} (regression)`, () => {
      const ctx = require('../lib/ssb-tennis-context');
...
describe('guessSurfaceFromFlashscoreMatch', () => {
  ...
  it('prefers explicit match.surface', () => {
    const { guessSurfaceFromFlashscoreMatch } = require('../lib/ssb-tennis-context');
...
describe('guessLevelFromFlashscoreMatch', () => {
  ...
  it('resolves Grand Slam from the tournament name for generic tour categories', () => {
    const { guessLevelFromFlashscoreMatch } = require('../lib/ssb-tennis-context');
    assert.equal(
...
describe('getTennisContext — flashscore exact-match fallback', () => {
  ...
  const TENNIS_PATH = require.resolve('../lib/ssb-tennis');
  let restoreEspn;
