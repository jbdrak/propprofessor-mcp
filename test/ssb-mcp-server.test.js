...
const {
  ...
  mapWithConcurrency
} = require('../scripts/ssb-mcp-server');
...
const serverPath = path.join(__dirname, '..', 'scripts', 'ssb-mcp-server.js');
...
describe('ssb MCP server stdio contract', () => {
  ...
  it('responds to initialize and lists the expected tools', async () => {
    const proc = spawn(process.execPath, [serverPath], {
      ...
      env: { ...process.env, SSB_MCP_MODE: 'full' }
    });
    ...
    try {
      ...
      assert.equal(initializeResponse.result.serverInfo.name, 'ssb');
  ...
  it('supports NDJSON initialize and tools/list framing when enabled', async () => {
    const proc = spawn(process.execPath, [serverPath], {
      stdio: ['pipe', 'pipe', 'pipe'],
      env: { ...process.env, SSB_MCP_NDJSON: 'true' }
    });
  ...
  it('returns structured categorized errors for failed tool calls', async () => {
    const server = createMcpServer({
      handlers: {
        fail_tool: async () => {
          const error = new Error('Missing SSB auth token');
    ...
    assert.equal(errObj.message, 'Missing SSB auth token');
  ...
  it('keeps a null tool throw inside the structured error envelope', async () => {
    ...
    assert.equal(errObj.message, 'Unexpected SSB MCP error');
  });
  ...
  it('screen_ranked returns a structured ranked response', async () => {
    ...
    // uses that as the movement source. The real SSB API would
...
describe('validated candidate concurrency helpers', () => {
  ...
  it('default rate limiter honors SSB_RATE_LIMIT and SSB_RATE_WINDOW_MS', async () => {
    const prevLimit = process.env.SSB_RATE_LIMIT;
    const prevWindow = process.env.SSB_RATE_WINDOW_MS;
    process.env.SSB_RATE_LIMIT = '1';
    process.env.SSB_RATE_WINDOW_MS = '2000';
    try {
      let handlerInvocations = 0;
      const server = createMcpServer({
        handlers: {
      ...
      if (prevLimit === undefined) delete process.env.SSB_RATE_LIMIT;
      else process.env.SSB_RATE_LIMIT = prevLimit;
      if (prevWindow === undefined) delete process.env.SSB_RATE_WINDOW_MS;
      else process.env.SSB_RATE_WINDOW_MS = prevWindow;
    }
  });

  it('bin entrypoints include node shebangs', () => {
    const serverEntry = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'ssb-mcp-server.js'), 'utf8');
    const queryEntry = fs.readFileSync(path.join(__dirname, '..', 'scripts', 'query-ssb.js'), 'utf8');
