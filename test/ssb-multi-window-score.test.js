...
const { computeMultiWindowScore, DEFAULT_WINDOWS, DEFAULT_SHARP_BOOKS } = require('../lib/ssb-sharp-consensus');
...
describe('multi-window score integration with risk grade and score', () => {
  const { gradeMovementQuality, calculateRiskScore } = require('../lib/ssb-risk-score');
...
describe('flat line handling (BUG fix: flat is no-signal, not adverse)', () => {
  it('getOddsDirection returns null for a flat line (current === opening)', () => {
    const { getOddsDirection } = require('../lib/ssb-sharp-consensus');
