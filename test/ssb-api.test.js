...
const { getLookbackHours, DEFAULT_ODDS_HISTORY_LOOKBACK_HOURS } = require('../lib/ssb-mcp-ranked-screen');
...
describe('buildSSBCookieHeader', () => {
  it('keeps only ssb cookies', () => {
    const header = buildSSBCookieHeader({
      cookies: [
...
describe('timeout helpers', () => {
  it('recognizes abort-like errors and tags timeout failures', () => {
    ...
    assert.equal(error.code, 'SSB_TIMEOUT_ERROR');
...
describe('fetchAccessToken', () => {
  ...
  it('mentions the resolved auth file path when no SSB cookies are present', async () => {
    const { dir, file } = makeTempAuthState({
    ...
    try {
      await assert.rejects(fetchAccessToken({ authFile: file }), (error) => {
        assert.match(error.message, /No SSB cookies found/);
        assert.match(
          error.message,
...
describe('createSSBClient', () => {
  ...
  it('deduplicates concurrent odds-history requests across clients', async () => {
    ...
    const clientA = createSSBClient({ ...clientOptions, authFile: auth.file });
    const clientB = createSSBClient({ ...clientOptions, authFile: auth.file });
    const params = {
    ...
    try {
      await assert.rejects(
        ...
        (error) => {
          ...
          assert.equal(error.code, 'SSB_BACKEND_ERROR');
          assert.equal(error.retryable, true);
          return true;
        }
      );
  ...
  it('preserves the original odds-history 429 throughout the shared cooldown', async () => {
    const { resetAllBreakers } = require('../lib/ssb-circuit-breaker');
    const { dir, file } = makeTempAuthState({
      cookies: [{ domain: '.propprofessor.com', name: 'session', value: 'cookie-value' }],
      origins: []
    });
  ...
  it('does not add a five-minute gate halt when the odds-history circuit is already open', async () => {
    const { getOrCreateBreaker, resetAllBreakers } = require('../lib/ssb-circuit-breaker');
    const { dir, file } = makeTempAuthState({
      cookies: [{ domain: '.propprofessor.com', name: 'session', value: 'cookie-value' }],
      origins: []
    });
  ...
  it('screen_ranked uses the shared ranked screen flow with hydration and freshness', async () => {
    ...
    const celticsRow = result.result.find((row) => row.selectionId === 'Moneyline:Boston_Celtics');
    assert.ok(celticsRow);
    assert.equal(celticsRow.lineHistoryAvailable, true);
    assert.equal(celticsRow.lineHistorySource, 'odds_history');
    assert.equal(celticsRow.historyGameId, 'game-ranked-1');
    assert.equal(celticsRow.normalizedSelectionId, 'Moneyline:Boston_Celtics');
    assert.equal(celticsRow.historyMatchedBy, 'selectionId');
    assert.equal(celticsRow.lineHistoryLookbackHours, 6);
    assert.equal(celticsRow.freshnessSource, 'updatedAt');
    assert.equal(celticsRow.freshnessFallbackUsed, false);
    assert.equal(typeof celticsRow.freshnessAgeMs, 'number');
    ...
    assert.equal(typeof celticsRow.openToCurrentClvPct, 'number');
    assert.equal(celticsRow.rankingProvenance.focusBook, 'NoVigApp');
    assert.equal(celticsRow.rankingProvenance.historyMatchedBy, 'selectionId');
    assert.equal(celticsRow.rankingProvenance.lineHistorySource, 'odds_history');
    assert.equal(celticsRow.rankingProvenance.normalizedSelectionId, 'Moneyline:Boston_Celtics');
  ...
  it('records exactly one circuit breaker failure for a terminal 429 response (no double count)', async () => {
    const { resetAllBreakers, getAllBreakersInfo } = require('../lib/ssb-circuit-breaker');
    const { dir, file } = makeTempAuthState({
      cookies: [{ domain: '.propprofessor.com', name: 'session', value: 'cookie-value' }],
      origins: []
    });
    ...
    try {
      await assert.rejects(
        ...
        (error) => {
          ...
          assert.equal(error.code, 'SSB_AUTH_ERROR');
  ...
  it('tags HTML checkpoint responses as transport errors', async () => {
    ...
    try {
      await assert.rejects(
        ...
        (error) => {
          assert.equal(error.category, 'transport');
          assert.equal(error.code, 'SSB_TRANSPORT_ERROR');
          assert.equal(error.status, 429);
          assert.equal(error.retryable, true);
