...
function clearModuleCache() {
  delete require.cache[require.resolve('../lib/ssb-player-context')];
}
...
describe('extractTweets', { concurrency: false }, () => {
  it('parses a SearchTimeline response into flat tweet objects', () => {
    const { extractTweets } = require('../lib/ssb-player-context');
...
describe('buildQuery', { concurrency: false }, () => {
  it('for Tennis, query is just the player name', () => {
    const { buildQuery } = require('../lib/ssb-player-context');
...
describe('searchX', { concurrency: false }, () => {
  it('returns a clean error when the subprocess throws null', async () => {
    ...
    try {
      const { searchX } = require('../lib/ssb-player-context');
