...
function freshRequire() {
  ...
  for (const key of Object.keys(require.cache)) {
    if (key.includes('ssb-source-authority')) {
      delete require.cache[key];
    }
  }
  return require('../lib/ssb-source-authority');
}
...
describe('portable watchlist path', () => {
  ...
  it('source contains no author-machine absolute path', () => {
    const src = fs.readFileSync(path.join(__dirname, '..', 'lib', 'ssb-source-authority.js'), 'utf8');
