...
const { CircuitBreakerOpenError } = require('../lib/ssb-circuit-breaker');
...
} = require('../lib/ssb-screen-history');
...
describe('hydrateScreenRowsWithHistory', () => {
  ...
  it('uses SharpOdds history after PP history is degraded and preserves execution quote', async () => {
    ...
    assert.equal(result.historyProvider, 'sharpodds');
    assert.equal(result.movementSourceBook, 'Pinnacle');
    assert.equal(result.odds, -110);
    assert.equal(result.currentOdds, -110);
    assert.equal(result.liquidityUsd, 500);
    assert.equal(result.lineHistory[1].line, 9);
  });

  it('replaces PP line-backfilled history with SharpOdds history', async () => {
    let providerCalls = 0;
    const client = makeClient(async () => [
      { odds: -110, start_ts: 1 },
      { odds: -115, start_ts: 2 }
...
describe('hydrateScreenRowsWithHistory — throttle/circuit fail-soft', () => {
  ...
  it('stops line-variant fallback after a 429 throttle error on the exact query', async () => {
    ...
    const client = makeClient(async () => {
      callCount += 1;
      const error = new Error('SSB backend failed (429): rate limited');
      error.status = 429;
      error.code = 'SSB_BACKEND_ERROR';
    ...
    assert.equal(result.historyErrorCode, 'SSB_BACKEND_ERROR');
