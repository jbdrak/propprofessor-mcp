...
const { fetchAccessToken, fetchAccessTokenViaCDP, fetchAccessTokenViaEgo } = require('../lib/ssb-auth');
...
describe('fetchAccessTokenViaCDP', () => {
  ...
  it('honors SSB_CDP_VERSION_URL when no option is passed', async () => {
    const previous = process.env.SSB_CDP_VERSION_URL;
    try {
      process.env.SSB_CDP_VERSION_URL = 'http://127.0.0.1:9333/json/version';
      let fetchedUrl = null;
      const fetchImpl = async (url) => {
        fetchedUrl = url;
        return {
      ...
      if (previous === undefined) delete process.env.SSB_CDP_VERSION_URL;
      else process.env.SSB_CDP_VERSION_URL = previous;
    }
  });

  it('explicit versionUrl wins over SSB_CDP_VERSION_URL', async () => {
    const previous = process.env.SSB_CDP_VERSION_URL;
    try {
      process.env.SSB_CDP_VERSION_URL = 'http://127.0.0.1:9333/json/version';
      let fetchedUrl = null;
  ...
  it('defaults to the 127.0.0.1:9222 version endpoint when nothing is configured', async () => {
    const previous = process.env.SSB_CDP_VERSION_URL;
    try {
      delete process.env.SSB_CDP_VERSION_URL;
      let fetchedUrl = null;
      const fetchImpl = async (url) => {
        fetchedUrl = url;
        return {
...
describe('fetchAccessToken — Vercel 429 self-heal via browser fallbacks (ego → CDP)', () => {
  ...
  it('does NOT call CDP when got-scraping returns 500 (non-retryable, non-Vercel)', async () => {
    ...
    await assert.rejects(
      ...
      /Failed to fetch SSB access token/
    );
...
describe('fetchAccessTokenViaEgo', () => {
  ...
  it('uses SSB_EGO_TASK_SPACE env var for the task space id', async () => {
    const previous = process.env.SSB_EGO_TASK_SPACE;
    try {
      process.env.SSB_EGO_TASK_SPACE = '42';
      const { FakeEgoSpawn, calls } = makeFakeEgoSpawn({
      ...
      if (previous === undefined) delete process.env.SSB_EGO_TASK_SPACE;
      else process.env.SSB_EGO_TASK_SPACE = previous;
    }
  });
  ...
  it('rejects invalid SSB_EGO_TASK_SPACE values', async () => {
    const previous = process.env.SSB_EGO_TASK_SPACE;
    try {
      process.env.SSB_EGO_TASK_SPACE = 'abc';
      await assert.rejects(
        fetchAccessTokenViaEgo({ spawnImpl: makeFakeEgoSpawn().FakeEgoSpawn }),
        /Invalid SSB_EGO_TASK_SPACE value/
      );
    } finally {
      if (previous === undefined) delete process.env.SSB_EGO_TASK_SPACE;
      else process.env.SSB_EGO_TASK_SPACE = previous;
