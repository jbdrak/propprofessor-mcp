...
const { extractEventLinkRows, chooseEventLink, groupEventLinks } = require('../lib/ssb-event-links');
  ...
  const rows = [
    {
      ...
      market: 'Moneyline',
      selection: 'Navarro',
      deepLink: 'https://novig.com/events/abc/ssb'
    },
    {
      book: 'NoVigApp',
      league: 'Tennis',
    ...
    },
    {
      ...
      market: 'Moneyline',
      selection: 'A',
      deepLink: 'https://novig.com/events/def/ssb'
    }
  ];

  const events = groupEventLinks(rows, { book: 'NoVigApp', leagues: ['Tennis'] });
  assert.equal(events.length, 1);
  assert.equal(events[0].eventLink, 'https://novig.com/events/abc/ssb');
