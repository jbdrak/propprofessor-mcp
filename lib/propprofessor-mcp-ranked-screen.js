...
const { buildResultMeta } = require('./propprofessor-ranked-screen-meta');
const { buildRankedResultRows } = require('./propprofessor-ranked-screen-results');
const { finalizeRankedScreenResponse } = require('./propprofessor-ranked-screen-finalize');
const { planPreHistoryHydration } = require('./propprofessor-ranked-screen-history-plan');
const { annotateRankedRows } = require('./propprofessor-ranked-screen-row-annotation');
const { buildRankedHydrationOptions } = require('./propprofessor-ranked-screen-hydration-options');
const { mergeRecoveredRows } = require('./propprofessor-ranked-screen-recovery-merge');
...
const { createSharpOddsHistoryProvider } = require('./sharpodds-history-provider');
...
async function buildRankedScreenResponse(opts = /** @type {any} */ ({})) {
  ...
  const enrichedRows = await enrichSoccerEventRows(extractedRows, {
        ...
        new Promise((resolve) => {
          timer = setTimeout(() => {
            ...
            process.stderr.write(
              `[propprofessor-mcp] History hydration hit the ${historyDeadlineMs}ms deadline; ` +
