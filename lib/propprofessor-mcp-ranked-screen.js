...
// sides = 42 history calls) exhausts the process-wide 75-call odds-history
// budget on low-value rows, and good late candidates degrade to
...
const PRE_HISTORY_SHORTLIST_MAX_GAMES = 24;
 ...
 * cap so one scan never approaches the process-wide 75-call budget.
 ...
 * row's expanded sides, with an implausible-edge penalty and a small bonus
 * for multi-book consensus. Never calls history.
 */
function scoreRowForShortlist(row, preferredBook) {
  ...
  try {
    ...
    for (const side of Array.isArray(expanded) ? expanded : []) {
      ...
      score += Math.min(2, Number(side.consensusBookCount) || 0) * 0.5;
 ...
 * @returns {{rows: Array<object>, gameCount: number, bucketCount: number, gameBudget: number}}
 */
...
async function buildRankedScreenResponse(opts = /** @type {any} */ ({})) {
  const { client, payloads = [], args = {}, league, rankRows, focusBook, resultMeta = {}, preHydrationFilter } = opts;
  const compact = getCompactFlag(args);
  const targetBook = String(focusBook || '').trim();
  const focusPlays = targetBook ? [{ book: targetBook }] : [];
  const extractedRows = payloads.flatMap((payload) => extractScreenRows(payload, focusPlays));
  const rows = typeof preHydrationFilter === 'function' ? extractedRows.filter(preHydrationFilter) : extractedRows;
  ...
  // 75-call odds-history budget on low-value rows and strong late candidates
  ...
  if (canHydrate && args.preHistoryShortlist === true && !hasExplicitGameFilter(args) && rows.length > 0) {
    ...
    if (shortlist.rows.length < rows.length) {
      ...
      preHistoryShortlistMeta = {
        enabled: true,
        // Honesty flag: rows outside the shortlist were NOT hydrated (and
        // therefore could not be ranked with movement data).
        truncated: true,
  ...
  const hydratedRows = skipHistory
    ? rows
    : await hydrateScreenRowsWithHistory(hydrationRows, {
        client,
