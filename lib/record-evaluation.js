...
const { assessSportContext } = require('./ssb-context-gates');
...
// Legacy calibration defaults (lib/ssb-signal-calibration.js used
 ...
 * modelMarketEdgePct.
 *
 * Optional fields (book, line, decisionTimestamp, closingOdds,
 * movementDisposition, movementSourceBook, correlationGroupId, sportContext,
 * sportContextAssessment) are attached only when a recorded value is present,
 * so legacy rows without those values keep the exact legacy shape.
 *
 * @param {Object} ledger - v2 ledger
...
function buildEvaluationRows(ledger) {
  ...
  for (const row of joinSettledBets(ledger)) {
    ...
    const book = pick(snapshot, row, 'book') ?? row.book ?? settlement.book ?? null;
    if (book != null) out.book = book;
    const line = pick(snapshot, row, 'line') ?? row.line ?? settlement.line ?? null;
    if (line != null) out.line = line;
    const decisionTimestamp = pick(snapshot, row, 'decisionTimestamp') ?? null;
    if (decisionTimestamp != null) out.decisionTimestamp = decisionTimestamp;
    const closingOdds = pick(snapshot, row, 'closingOdds') ?? settlement.closingOdds ?? null;
    if (closingOdds != null) out.closingOdds = closingOdds;
    const movementDisposition = pick(snapshot, row, 'movementDisposition') ?? null;
    if (movementDisposition != null) out.movementDisposition = movementDisposition;
    const movementSourceBook = pick(snapshot, row, 'movementSourceBook') ?? null;
    if (movementSourceBook != null) out.movementSourceBook = movementSourceBook;
    const correlationGroupId = pick(snapshot, row, 'correlationGroupId') ?? null;
    if (correlationGroupId != null) out.correlationGroupId = correlationGroupId;
    if (snapshot && snapshot.sportContext && typeof snapshot.sportContext === 'object') {
      out.sportContext = clone(snapshot.sportContext);
      out.sportContextAssessment = assessSportContext({
        league: out.league,
        market: out.market,
        sportContext: out.sportContext
      });
    }
    rows.push(out);
  }
...
const DEFAULT_SEGMENT_DIMENSIONS = Object.freeze(['league', 'market']);
const DECISION_LEAK_FIELDS = Object.freeze([
  'outcome',
  'result',
  'winner',
  'finalScore',
...
function resolveSegmentOptions(opts = {}) {
  const dimensions =
    Array.isArray(opts.dimensions) && opts.dimensions.length > 0
      ? [...new Set(opts.dimensions.map((value) => String(value)))]
      : [...DEFAULT_SEGMENT_DIMENSIONS];
  const minSample = opts.minSample === undefined ? DEFAULT_MIN_SAMPLE : opts.minSample;
  if (!Number.isInteger(minSample) || minSample <= 0) {
    throw new TypeError(`minSample must be a positive integer, got ${String(minSample)}`);
  }
  return { dimensions, minSample };
}

/**
 * Segment settled evaluation rows without mixing sports or markets.
 * Unknown outcome rows are ignored because they cannot be evaluated.
 * @param {Array<Object>} rows
 * @param {{dimensions?: string[], minSample?: number}} [opts]
 * @returns {{dimensions: string[], segments: Object<string, Object>}}
 */
function segmentEvaluationRows(rows, opts = {}) {
  const { dimensions, minSample } = resolveSegmentOptions(opts);
  const segments = {};
  for (const row of Array.isArray(rows) ? rows : []) {
    if (!row || !['win', 'loss', 'push'].includes(row.outcome)) continue;
    ...
    if (!segments[key]) {
      segments[key] = {
        values: Object.fromEntries(dimensions.map((dimension, index) => [dimension, values[index]])),
        wins: 0,
        losses: 0,
        pushes: 0,
        sampleSize: 0,
    ...
    segment[row.outcome === 'win' ? 'wins' : row.outcome === 'loss' ? 'losses' : 'pushes'] += 1;
    segment.sampleSize += 1;
    if (row.outcome !== 'push') segment.totalDecided += 1;
  }
  for (const segment of Object.values(segments)) {
    segment.hitRate = segment.totalDecided > 0 ? segment.wins / segment.totalDecided : null;
    segment.insufficientSample = segment.totalDecided < minSample;
  }
  return { dimensions, segments };
}

/**
 * Detect post-decision data leaking into an evaluation feature snapshot.
 * @param {Object} row
 * @returns {{ok: boolean, violations: string[]}}
 */
function validateDecisionTimeIntegrity(row = {}) {
  const violations = [];
  const snapshot = row.featureSnapshot && typeof row.featureSnapshot === 'object' ? row.featureSnapshot : {};
  for (const field of DECISION_LEAK_FIELDS) {
    if (Object.prototype.hasOwnProperty.call(snapshot, field)) violations.push(`featureSnapshot.${field}`);
  }
  const decisionTimestamp =
    row.decisionTimestamp ?? snapshot.decisionTimestamp ?? row.capturedAt ?? snapshot.capturedAt;
  const settledAt = row.settledAt;
  if (decisionTimestamp && settledAt) {
    const decisionMs = Date.parse(decisionTimestamp);
    const settledMs = Date.parse(settledAt);
    if (Number.isFinite(decisionMs) && Number.isFinite(settledMs) && decisionMs >= settledMs) {
      violations.push('decisionTimestamp_not_before_settledAt');
    }
  }
