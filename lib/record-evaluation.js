...
function tennisOf(snapshot) {
  return snapshot && snapshot.tennis && typeof snapshot.tennis === 'object' && !Array.isArray(snapshot.tennis)
    ? snapshot.tennis
    : null;
}
 ...
 * derived. Explicit tennis Elo fields are surfaced only when the decision-
 * time snapshot carried them.
 *
 ...
 * modelMarketEdgePct, tennisSurface, tennisTour, tennisElo, tennisCoverage,
 * tennisFreshness, tennisModelVersion.
 *
 * @param {Object} ledger - v2 ledger
...
function buildEvaluationRows(ledger) {
  ...
  for (const row of joinSettledBets(ledger)) {
    ...
    const tennis = tennisOf(snapshot);
    ...
    rows.push({
      ...
      tennisSurface: tennis ? (tennis.surface ?? null) : null,
      tennisTour: tennis ? (tennis.tour ?? null) : null,
      tennisElo: tennis && tennis.elo != null ? clone(tennis.elo) : null,
      tennisCoverage: tennis ? (tennis.coverage ?? null) : null,
      tennisFreshness: tennis ? (tennis.freshness ?? null) : null,
      tennisModelVersion: tennis ? (tennis.modelVersion ?? null) : null
    });
  }
  return rows;
}
