...
function normalizeMarket(market) {
  const upper = String(market || '').trim().toUpperCase();
  ...
  try {
    ...
    try {
      ...
      if (thrown && thrown.name === 'AbortError') {
        throw sharpError({ endpoint: url, category: 'timeout', message: `sharpodds-client: request timed out after ${timeoutMs}ms` });
      }
      throw sharpError({ endpoint: url, category: 'network', message: `sharpodds-client: request failed (${(thrown && thrown.message) || 'network error'})` });
    }
    const status = res && typeof res.status === 'number' ? res.status : 0;
    const ok = res && (typeof res.ok === 'boolean' ? res.ok : status >= 200 && status < 300);
    if (!ok) {
      throw sharpError({ endpoint: url, status, category: 'http', message: `sharpodds-client: HTTP ${status} from SharpOdds odds endpoint` });
    }
    let data;
    try {
      data = await res.json();
    } catch (thrown) {
      throw sharpError({ endpoint: url, status, category: 'parse', message: `sharpodds-client: malformed JSON (${(thrown && thrown.message) || 'parse error'})` });
    }
...
async function fetchBoard(fetchImpl, params, options) {
  ...
  let query = params;
  let opts = options;
  if (query && typeof query === 'object' && (query.timeoutMs !== undefined || query.signal !== undefined) && opts === undefined) {
