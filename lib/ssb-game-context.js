...
const { getMlbGameContext } = require('./ssb-mlb-game-context');
const { getBasketballGameContext } = require('./ssb-basketball-game-context');
const { getNhlContext } = require('./ssb-nhl-context');
const { getTennisContext } = require('./ssb-tennis-context');
const { getUfcContext } = require('./ssb-ufc-context');
...
async function getGameContext(opts = {}) {
  const { sport, selection, game, start } =
    /** @type {{ sport: string, selection?: string, game?: string, start?: string }} */ (opts);
  ...
  switch (csport) {
    case 'MLB': {
      try {
        const { findMlbGamePk } = require('./ssb-mlb-game-context');
        let gamePk = null;
