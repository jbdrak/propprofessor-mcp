...
function resolveSharpPlaySignals(row = {}, options = {}) {
  ...
  const mainstreamFallback = row.historyBookTier === 'mainstream_fallback';
  ...
  const sharpMovementMode =
    !mainstreamFallback && (row.movementMode === 'same_book' || row.movementMode === 'comparison_book');
...
function classifySharpPlay(row = {}, options = {}) {
  ...
  if (!sharpBookMovementConfirmed) {
    if (mainstreamFallback) passReasons.push('mainstream_fallback_movement');
