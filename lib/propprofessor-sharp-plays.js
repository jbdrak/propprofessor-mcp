...
function classifySharpPlay(row = {}, options = {}) {
  ...
  //
  // EXCEPTION — consensusValidated (restored from pre-dbc7636): no-vig books
  // (NoVigApp, OnyxOdds) are themselves the sharp reference, so their own
  // line movement is the sharp signal. A row with strong multi-book
  // consensus AND a positive consensus edge AND supportive movement label —
  // even when the only movement source is the target book itself
  // (movementMode: same_book) — is a legitimate Bet candidate. Without this,
  // every non-tennis league on a no-vig book scan degrades to
  // movement_source_is_target_book and the slate looks dead.
