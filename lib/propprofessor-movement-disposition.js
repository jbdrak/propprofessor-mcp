...
function computeMovementDisposition(row) {
  ...
  // Stale-quote gate: never present a supportive disposition as a live signal
  // when the newest history point is older than STALE_QUOTE_MAX_AGE_MS. A
  // supportive label on an aged quote is exactly the shape that misleads
  // (quote looks clean, market already moved). Adverse dispositions stay —
  // they are a PASS either way, and downgrading them would only hide
  // information.
  //
  ...
  if (
    ...
    disposition.startsWith('supportive_')
  ) {
