 ...
 *   "stale"                 — history is too old for a current movement read.
...
// A quote whose newest line-history point is older than this is NOT a current
// price. Movement labels built from it must not read as a live, clean signal —
// the market can (and does) move in the gap, flipping supportive to adverse.
// Exact-lookup history points carry real `time` timestamps; when those are
// missing, lastPointAgeMs is null and this gate is skipped (existing freshness
// handling applies). Verified 2026-08-14: Broncos ML -178 was quoted as
// supportive_clean from a 17-minute-old history point while the live market
// had already moved to -163/-160 adverse.
const STALE_QUOTE_MAX_AGE_MS = 10 * 60 * 1000;

function computeMovementDisposition(row) {
  if (!row || typeof row !== 'object') return 'insufficient';
  if (row.lineHistoryUsable === false) return 'insufficient';

  // Stale-quote gate: never present a stale movement label as a definitive
  // live signal when the newest history point is older than the threshold.
  // This applies to adverse labels too: an old, choppy move is not proof of
  // current sharp pressure and must not be overstated as adverse_full.
  //
  // Corroboration escape (2026-08-15, UFC 330): a stale quote whose
  // supportive read is independently confirmed by 5+ books agreeing AND a
  // non-negative CLV is still a valid signal — the market hasn't gone quiet,
  // the movement SOURCE book's feed just stopped pushing (OnyxOdds quiet for
  // 2h+ while Kalshi/theScore/Caesars pushed fresh prices 10 min ago). Only
  // downgrade to insufficient when the supportive read is UNcorroborated.
  // Mirrors the strongConsensus + clvOk logic in insufficient().
  const disposition = computeMovementDispositionUnchecked(row);
  const lastPointAgeMs = Number(row.lastPointAgeMs);
  if (
    Number.isFinite(lastPointAgeMs) &&
    lastPointAgeMs > STALE_QUOTE_MAX_AGE_MS &&
    (disposition.startsWith('supportive_') || disposition.startsWith('adverse'))
  ) {
    if (disposition.startsWith('adverse')) return 'stale';
    const clvPct = Number(row.clvProxyPct ?? row.clv);
    const corroborated =
      (Number(row.consensusBookCount) || 0) >= MOVEMENT_DISPOSITION_SUPPORT && Number.isFinite(clvPct) && clvPct >= 0;
    if (!corroborated) {
      return 'stale';
    }
  }
...
function computeMovementDispositionUnchecked(row) {
  let grade = String(row.movementGrade || '').toLowerCase();
  ...
  // Provenance of the confirming book. An originator (Pinnacle/Circa/BookMaker/
  // BetOnline) moves FIRST on informed action; a follower (DraftKings/FanDuel/
  // BetMGM) copies it seconds later, by which point the value may be gone. A
  // confirmation from an originator is the stronger signal. Exchanges and
  // derived no-vig books are labeled separately. Never fabricated — resolved
  // from the row's own sharpBookMovementOrigin field or the source book name
  // via classifySharpBookOrigin (see getSharpMovementOrigin).
  ...
  //    BUT grade red also fires for bad execution quality (off-market price
  //    on the target book) — that is a playability failure, not a movement
  //    signal. Only stamp adverse_full when the movement data corroborates
  //    adverse (label or a direction window says adverse). A red grade on
  //    supportive movement means "unplayable quote," so grade the row on
  //    its movement fields (yellow) instead of calling sharp money against.
  if (grade === 'red') {
    const movementAdverse = fullLabel === 'adverse' || recentDir === 'adverse' || fullDir === 'adverse';
    if (movementAdverse) return 'adverse_full';
    grade = 'yellow';
  }
...
function computeMovementSummary(row, extra = {}) {
  ...
  const sharpOrigin = getSharpMovementOrigin(row);
  ...
  if (disp.startsWith('supportive_clean')) {
    ...
    // Surface which book confirmed it — an originator confirmation (Pinnacle/
    // Circa/BookMaker/BetOnline moving first) is the strongest clean-move signal.
    if (sharpOrigin === 'originator' && sharpSource) {
      parts.push(`— sharp-originator confirmation (${sharpSource})`);
    } else if (sharpSource) {
      parts.push(`— confirmed by ${sharpSource}`);
    }
    return parts.join(' ');
  }
  ...
  if (disp === 'stale') {
    const ageMinutes = Number.isFinite(Number(row.lastPointAgeMs))
      ? Math.round(Number(row.lastPointAgeMs) / 60000)
      : null;
    return ageMinutes ? `Stale movement history (${ageMinutes} min old)` : 'Stale movement history';
  }

  if (disp === 'insufficient') {
    if (row.sharpBookMovementConfirmed) {
      if (sharpOrigin === 'originator' && sharpSource) {
        return `Movement inferred from sharp originator ${sharpSource} (thin in-row history)`;
      }
      return sharpSource
...
module.exports = { computeMovementDisposition, computeMovementSummary, getSharpMovementOrigin };

/**
 * Resolve the line-movement origin tier of the confirming book for a row.
 * Returns '' when no sharp book confirmed the move.
 * @param {Object|null} row - Enriched screen row
 * @returns {'originator'|'exchange'|'derived'|'follower'|''}
 */
function getSharpMovementOrigin(row) {
  if (!row || typeof row !== 'object') return '';
  const explicit = String(row.sharpBookMovementOrigin || '').toLowerCase();
  if (explicit) return /** @type {'originator'|'exchange'|'derived'|'follower'|''} */ (explicit);
  if (!row.sharpBookMovementSource) return '';
  return /** @type {'originator'|'exchange'|'derived'|'follower'} */ (
    classifySharpBookOrigin(row.sharpBookMovementSource)
  );
}
