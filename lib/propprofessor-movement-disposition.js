...
function computeMovementDisposition(row) {
  ...
  if (
    ...
    const corroborated =
      (Number(row.consensusBookCount) || 0) >= MOVEMENT_DISPOSITION_SUPPORT &&
...
function computeMovementDispositionUnchecked(row) {
  ...
  if (grade === 'red') {
    const movementAdverse =
      fullLabel === 'adverse' || recentDir === 'adverse' || fullDir === 'adverse';
...
function computeMovementSummary(row, extra = {}) {
  ...
  if (disp === 'stale') {
    const ageMinutes = Number.isFinite(Number(row.lastPointAgeMs)) ? Math.round(Number(row.lastPointAgeMs) / 60000) : null;
