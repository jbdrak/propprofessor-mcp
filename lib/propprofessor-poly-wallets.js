...
function classifyPosition(play, stance) {
  ...
  const normTitle = normForMatch(title);
  if (!outcomeNorm || !normTitle.includes(selectionNorm)) return null;
 ...
 * @returns {Promise<Array<{conditionId, title, outcome, dollar}>>}
 */
async function fetchWalletStances(userAddr, opts) {
  ...
  try {
    ...
    const net = new Map(); // key: conditionId|outcome -> { title, outcome, dollar, ts }
    for (const t of data) {
      ...
      const prev = net.get(key) || { conditionId, title, outcome, dollar: 0, ts: 0 };
      prev.dollar += side === 'BUY' ? usdc : -usdc;
      prev.ts = Math.max(prev.ts, Number(t.timestamp) || 0);
      net.set(key, prev);
    }
    ...
    return [...net.values()]
      ...
      .map((s) => ({ conditionId: s.conditionId, title: s.title, outcome: s.outcome, dollar: Math.round(s.dollar) }));
  } catch {
    return [];
  }
}
