...
// True when AbortSignal.any is available (Node >= 20). Used to compose a
// caller-supplied signal with our timeout without losing the parent abort.
const HAS_ABORT_SIGNAL_ANY = typeof AbortSignal === 'function' && typeof AbortSignal.any === 'function';

/**
 * Build an AbortSignal that fires after `ms`. If the caller passes its own
 * signal we compose the two so either abort cancels the request. Returns the
 * caller's signal unchanged (or null) on runtimes without AbortSignal.timeout,
...
function timeoutSignal(ms, parentSignal) {
  if (typeof AbortSignal !== 'function' || typeof AbortSignal.timeout !== 'function') {
    return parentSignal || null;
  }
  if (parentSignal && HAS_ABORT_SIGNAL_ANY) {
...
async function fetchLeaderboard(limit, opts) {
  ...
  try {
    const res = await fetchImpl(LEADERBOARD_URL + '?limit=' + MAX_WALLET_LIMIT, {
      ...
      signal: timeoutSignal(o.timeoutMs == null ? FETCH_TIMEOUT_MS : o.timeoutMs, o.signal)
    });
...
async function fetchWalletStances(userAddr, opts) {
    ...
    if (positionsRes && positionsRes.ok) {
      ...
      if (
        ...
        (data.length === 0 ||
          data.some(
            (p) =>
              p && p.conditionId && p.title && p.outcome && ('size' in p || 'currentValue' in p || 'initialValue' in p)
          ))
      ) {
        return data
  ...
  try {
    const res = await fetchImpl(ACTIVITY_URL + '?user=' + encodeURIComponent(userAddr) + '&limit=' + ACTIVITY_LIMIT, {
      headers: { Accept: 'application/json' },
      signal: timeoutSignal(o.fallbackTimeoutMs == null ? FALLBACK_FETCH_TIMEOUT_MS : o.fallbackTimeoutMs, o.signal)
    });
    if (!res || !res.ok) return [];
    const data = await res.json();
