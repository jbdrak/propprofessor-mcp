...
//
// Source note (learned 2026-08-16 while building): the /positions endpoint
// returns RESOLVED markets only (redeemable:true, curPrice:0) for these
// wallets — useless for live holdings. The /activity feed carries live
// TRADE rows with title, outcome, side, usdcSize, so v1 derives a wallet's
// current stance by netting BUY vs SELL usdc per (conditionId, outcome).
// Positive net = still holds that side.
//
 ...
 *
 * Stances on settled (closed) markets are dropped: a losing position has no
 * REDEEM row, so without this a settled-loss shows as a stale live position.
 * `fetchClosedMarkets` probes the CLOB `/markets` endpoint per conditionId
 * (degrades to "live" on any fetch failure).
 */
async function fetchWalletStances(userAddr, opts) {
  ...
  try {
    ...
    // Drop stances on settled markets: a losing position has no REDEEM row,
    // so without this a settled-loss shows as a stale "live" position. The
    // CLOB market endpoint carries `closed` (market has ended) — treat any
    // market that's no longer accepting orders as settled and drop its stances.
    const liveCids = new Set();
    for (const key of net.keys()) {
      liveCids.add(key.split('|')[0]);
    }
    const closedCids = await fetchClosedMarkets(Array.from(liveCids), fetchImpl);
    for (const key of net.keys()) {
      if (closedCids.has(key.split('|')[0])) net.delete(key);
    }
    return [...net.values()]
...
/**
 * Batch-lookup which conditionIds map to settled (closed) Polymarket markets.
 * Uses the CLOB `/markets` endpoint: one request per conditionId since the
 * endpoint is per-market. Returns a Set of closed conditionIds.
 * Degrades to [] (treat everything as live) on any fetch/JSON failure so a
 * transient API hiccup never swallows real wallet positions.
 */
async function fetchClosedMarkets(conditionIds, fetchImpl) {
  if (!Array.isArray(conditionIds) || conditionIds.length === 0 || typeof fetchImpl !== 'function') {
    return new Set();
  }
  ...
  await Promise.all(
    conditionIds.map(async (cid) => {
      if (!cid) return;
      try {
        const res = await fetchImpl(CLOB_API + '/markets/' + encodeURIComponent(cid), {
          headers: { Accept: 'application/json' }
        });
        if (!res || !res.ok) return;
        const m = await res.json();
        if (m && m.closed === true) closed.add(cid);
      } catch {
        // a single market lookup failing leaves it treated as live
      }
    })
  );
  return closed;
