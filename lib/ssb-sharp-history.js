...
const { americanOddsToImpliedProbability, parseHistoryTimeMs } = require('./ssb-shared-utils');

function normalizeBook(value) {
  return String(value || '').trim();
}

function isDerivedBook(value) {
  return new Set(['novigapp', 'onyxodds']).has(normalizeBook(value).toLowerCase());
}
...
function pickMovementSource(
  ...
  // Sharp-book movement is the signal. When the target book is itself a sharp
  // book (e.g. -b Pinnacle), its own line IS the sharp signal -> same_book.
  // When the target is a no-vig/consumer book (NoVigApp, OnyxOdds), its own
  // line is NOT sharp — prefer actual sharp books (Pinnacle/Circa/BetOnline)
  // so the label reflects where sharp money moved, not the flat no-vig line.
  const preferredIsSharp = preferred && sharpBookList.includes(preferred);

  if (preferredIsSharp && groupedPoints[preferred]?.length >= 2) {
  ...
  for (const book of namedBooks) {
    if (book === preferred && !preferredIsSharp && isDerivedBook(preferred)) continue;
...
function getMovementQuality({ movementMode, movementSourceBook, sharpBooks = [], movementPointCount, droppedCount }) {
  ...
  // A comparison-book trail is still high-quality when it comes from a
  // configured sharp book. The target book is only the execution quote; it
  // should not downgrade independent sharp movement by itself.
  ...
  if (movementMode === 'comparison_book') {
    return { movementQuality: 'low', movementQualityScore: 0.35 };
  }
  if (pointCount >= 2) {
...
function summarizeSharpMovement({ lineHistory, preferredBook, sharpBooks, options = /** @type {any} */ ({}) }) {
  ...
  const timestampsUsable =
    source.points.length >= 2 && source.points.every((point) => Number.isFinite(parseHistoryTimeMs(point?.time)));
  const lineFieldMissingCount = Number(options.lineFieldMissingCount || 0);
  const lineHistoryQuality = lineFieldMissingCount > 0 ? 'degraded_line_fields' : 'verified';
  const priceHistoryUsable =
    options.priceHistoryUsable === true ||
    (options.priceHistoryUsable !== false &&
      lineFieldMissingCount === 0 &&
      timestampsUsable &&
      Boolean(windows.fullWindow && Number.isFinite(windows.fullWindow.clvPct)));
  const movementHistoryUsable =
    priceHistoryUsable && !options.historyDegraded && source.movementMode !== 'mixed_books_fallback';
  const movementProvenanceUsable = movementHistoryUsable && lineFieldMissingCount === 0;
  const lineHistoryUsable =
    movementProvenanceUsable && Boolean(windows.fullWindow && Number.isFinite(windows.fullWindow.clvPct));
  const movementLabel = getMovementLabel({
    fullDirection,
    recentDirection,
    lineHistoryUsable: movementHistoryUsable
  });
