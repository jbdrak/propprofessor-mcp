...
function applyValidatedFields(target, validationResult) {
  ...
  // Keep movement risk flags aligned with the reconciled disposition. The
  // validator's raw riskFlags can contain "movement adverse" even when the
  // re-fetch flip was rejected as classifier/book-set noise by reconcile.
  const rawRiskFlags = Array.isArray(verdict.riskFlags) ? verdict.riskFlags : [];
  target.validatedRiskFlags =
    reconcile.overridden && reconcile.movementDisposition.startsWith('supportive_')
      ? rawRiskFlags.filter((flag) => flag !== 'movement adverse')
      : rawRiskFlags;
