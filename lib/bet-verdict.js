 ...
 *
 * Dependencies: none. Phase 3A removed reconcileValidateOverride —
 * validator output passes through verbatim; fail-closed guards below
 * (adverse/insufficient/bad -> downgrade) are the only safety net.
 */
...
function applyValidatedFields(target, validationResult) {
  ...
  // Phase 3A: no reconciliation. The validate path (confirmExecution) already
  // trusts the screen snapshot for scan-sourced rows, so re-reconciling here
  // only reintroduces flip-flop on Novig quote noise. Pass through verbatim;
  // degraded rows stay non-actionable via the applyFinalVerdict guards below.
  target.validatedMovementDisposition =
    verdict.movementDisposition || target.movementDisposition;
  target.validatedExecQuality = play.executionQuality || target.executionQuality;
  // Deprecated compat keys (Phase 4 removes): constant so old
  // !== true checks fail closed.
  target.validatedReconcileOverridden = false;
  target.validatedReconcileReason = 'reconciliation removed (Phase 3A): validator output passed through verbatim';
  target.validatedRiskFlags = Array.isArray(verdict.riskFlags) ? verdict.riskFlags : [];
...
function applyFinalVerdict(target) {
  ...
  // Fix: ranker sometimes stamps kaiCall=BET with confidenceTier=TIER 4
  // (when disposition is 'insufficient' but other signals are strong).
  // A BET verdict with TIER 4 is contradictory — clamp to TIER 2 so the
  // onlyBets tier filter (floor=1 for TIER 2) doesn't silently drop it.
  if (verdict === 'BET' && validatedTier === 'TIER 4') {
    validatedTier = 'TIER 2';
  }

  // Fix: when confidenceTier is null (ranker stamped kaiCall but no tier),
  // derive tier from kaiCall so BET/CONSIDER rows don't get silently dropped
  // by the onlyBets tier filter (which kills TIER 4 rows).
  if (!target.confidenceTier && !target.validatedConfidenceTier) {
    if (verdict === 'BET') validatedTier = 'TIER 2';
    else if (verdict === 'CONSIDER') validatedTier = 'TIER 3';
    else validatedTier = 'TIER 4';
  }
  ...
  const validationFailed =
    ...
    (!target._validated && !validatedVerdict && target.validationSkipped !== true && !target.validationBudgetSkipped) ||
    (Array.isArray(target.finalWarnings) && target.finalWarnings.includes('validation-failed') && target.validationBudgetSkipped !== true);
  ...
  if (!target._validated && target.validationSkipped !== true && target.validationBudgetSkipped !== true) {
...
function flagContradictoryPlays(plays) {
  ...
  for (const p of plays) {
    // Only two surviving BETs are contradictory. PASS/CONSIDER rows can
    // retain supportive movement labels from earlier stages, but they are not
    // opposing bets and must not demote the actionable side.
    const verdict = String(p.finalVerdict || p.kaiCall || p.displayTier || '').toUpperCase();
    if (verdict !== 'BET') continue;
