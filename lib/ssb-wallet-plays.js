...
// lib/ssb-wallet-plays.js — pure resolver + matcher + verdict for
...
//     (ssb-market-registry); stance math is pure string work.
//   - The fetch layer (leaderboard + per-wallet stances) lives in
//     ssb-poly-wallets and is bound via the module object at CALL
...
//   - Matching is deliberately conservative, mirroring the wallet overlay
//     philosophy: exact selection first, then game-containment with an exact
//     side-field tiebreaker.

const polyWallets = require('./ssb-poly-wallets');
const { getMarketsForSport, MARKET_REGISTRY } = require('./ssb-market-registry');
...
function leagueMarketName(league, kind) {
  ...
  if (kind === 'moneyline') {
    const hit = list.find((m) => /moneyline|draw no bet/i.test(m));
    return hit || 'Moneyline';
  }
  if (kind === 'total') {
...
const TOTAL_LINE_RE = /(?:o\/u|over|under)\s*(\d+(?:\.\d+)?)/i;
...
function parseSpreadLine(title) {
  ...
  const parenthesized = SPREAD_LINE_RE.exec(text);
...
function verdictForRow(row) {
  ...
  if (mov === 'stale') {
    return { verdict: 'PASS', reason: 'stale movement history' };
  }
...
/** Best-effort spread line from the row's explicit fields or selection text. */
function rowSpreadLine(row) {
  for (const value of [row?.line, row?.handicap, row?.points]) {
    if (value === null || value === undefined || String(value).trim() === '') continue;
    const parsed = Number(value);
    if (Number.isFinite(parsed)) return parsed;
  }
  const m = /(?:^|\s)([+-]\d+(?:\.\d+)?)(?=\s|$)/.exec(String(row?.selection || ''));
  return m ? Number(m[1]) : null;
}
...
function matchStanceToRow(stance, rows) {
  ...
  const spreadLineMatches = (row) => {
    if (marketKind !== 'spread' || line === null || line === undefined) return true;
    const rowLine = rowSpreadLine(row);
    return rowLine === null || Math.abs(rowLine - line) <= 0.001;
  };
  for (const row of pool) {
    if (!spreadLineMatches(row)) continue;
...
/**
 * Resolve one wallet row's stances and line each up with ranked rows from
 * rowsFor(league, marketName). Returns the wallet plus its resolvable stances,
 * or null when the row yields no league/matched stances (filtered out upstream).
 *
 * This JSDoc documents buildWalletStanceEntry — the previous comment block
 * described analyzeWalletPlays and was misattached here, which made the
 * TypeScript checker infer the aggregate return type across the wrong function
 * and error on the real `{ wallet, stances }` shape. See analyzeWalletPlays for
 * the top-level orchestration and its aggregate return contract.
 *
 * @param {object} row - one wallet entry from the stances feed
 * @param {object} deps
 * @param {(league: string, marketName: string) => Promise<Array<object>>} deps.rowsFor
 *   - ranked/broad rows for a (league, marketName) pair
 * @param {Function} [deps.exactFn] - async (league, marketName, gameId, book) ->
 *   exact per-game rows (authoritative tier/movement). When provided, matched
 *   stances are re-graded against these exact rows so degraded scan labels
 *   never dictate a verdict.
 * @param {string} [deps.book='NoVigApp'] - book for exact re-grade parity
 * @param {string|null} [deps.wantLeague] - only consider stances in this league
 *   (canonical 'Tennis', 'MLB', ...; case-insensitive); null = any
 * @param {string|null} [deps.wantDate] - only consider stances whose eventSlug
 *   carries this trailing date ('YYYY-MM-DD'); non-parsing slugs are dropped
 * @param {Map<string, { prefix: string, count: number, example: string }>} deps.droppedByPrefix
 *   - diagnostic accumulator for stances dropped by unmapped slug prefix
 * @returns {Promise<{ wallet: { proxyWallet: string|null, userName: string|null, pnl: number|null }, stances: Array<object> }|null>}
 */
async function buildWalletStanceEntry(row, { rowsFor, exactFn, book, wantLeague, wantDate, droppedByPrefix }) {
  if (!row || !row.wallet || !Array.isArray(row.stances)) return null;
  ...
  for (const group of groups.values()) {
    ...
    for (const { stance, resolved } of group.stances) {
      ...
      const broadRow = match.matched ? match.row : undefined;
      let displayRow = broadRow;
      let baseVerdict = broadRow ? verdictForRow(broadRow) : undefined;
      if (matched && typeof exactFn === 'function') {
        matched = false;
        displayRow = undefined;
        baseVerdict = undefined;
        if (broadRow && broadRow.gameId) {
          try {
            const exactRows = await exactFn(resolved.league, marketName, broadRow.gameId, book);
            ...
            // Fail closed. The broad scan row is not authoritative.
          }
        }
      }
      stancesOut.push({
        ...
        marketName,
        row: displayRow,
        exact: matched && displayRow !== broadRow,
        verdict: baseVerdict
      });
    }
  }
 ...
 * ranked rows. Only wallets with at least one matched stance are returned.
 *
 * @param {object} [opts]
 ...
 * @returns {Promise<{ wallets: Array<{ wallet: { proxyWallet: string|null, userName: string|null, pnl: number|null }, stances: Array<object> }>, gameTallies: Array<{ game: string, side: string, usd: number, wallets: number }>, droppedByPrefix: Array<{prefix: string, count: number, example: string}>, nonSportsDropped: number }>}
 */
async function analyzeWalletPlays(opts) {
  ...
  for (const row of walletRows) {
    const entry = await buildWalletStanceEntry(row, {
      rowsFor,
      exactFn,
      book,
      wantLeague,
