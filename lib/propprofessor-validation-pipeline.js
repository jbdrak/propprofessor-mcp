 ...
 * @param {(target: Object) => void} opts.onNotSelected - Called for each BET row
...
async function runValidationPipeline({
  ...
  for (const { target, entry } of rows) {
    ...
    if (!selectedSet.has(target) && isBet(target)) onNotSelected(target);
    ...
    validationPromises.push(
      (async () => {
        try {
          const validatePromise = validate(buildArgs(target, entry));
          let timeoutId;
          const timeoutPromise = new Promise((_, reject) => {
            timeoutId = setTimeout(
              () => reject(new Error(`Validation timeout for ${target.gameId}:${target.selection}`)),
              timeoutMs
            );
          });
          try {
            const result = await Promise.race([validatePromise, timeoutPromise]);
            if (target.gameId && result && result.ok) {
          ...
          target.validationFailureReason = errorMessage;
          return { target, result: null, error: errorMessage };
        }
      })()
    );
  }

  const validationResults = await mapWithConcurrency(validationPromises, async (p) => p, { concurrency });
...
module.exports = { runValidationPipeline, selectTopGlobal, selectTopPerBucket };
