...
const { compactRow, DEFAULT_LEAGUES, parseGameStartMs } = require('./ssb-shared-utils');
const { getLocalTimezone, localDateKey } = require('./mcp-runtime-config');
const { getMarketsForSport } = require('./ssb-market-registry');
...
/**
 * Resolve league list from arguments, defaulting to every league the SSB backend supports: NBA, MLB, NFL, NHL, WNBA, NCAAB, NCAAF, Soccer, Tennis, UFC.
...
/**
 * Resolve the input-derived sharp-play gates once. Keeping this private module
 * separate gives the public classifier one place for verdicts and pass reasons.
 */
function resolveSharpPlaySignals(row = {}, options = {}) {
  ...
  // comparison_book means the target book lacked usable history but an
  // independent sharp book supplied it. The execution quote stays on target,
  // so this is still sharp-sourced movement. Other fallback modes are not.
  const sharpMovementMode = row.movementMode === 'same_book' || row.movementMode === 'comparison_book';
  const movementIsSharpSourced = Boolean(
    (movementHistoryUsable && sharpMovementMode && movementSourceBook && independentBookOk && movementLabelSupported) ||
  ...
  // No-vig books can be the sharp reference themselves. A strong positive
  // consensus plus a supportive target-book move is therefore a valid signal,
  // even when independent movement is absent.
  const consensusValidated =
    consensusOk && hasConsensusEdge && movementLabelSupported && !movementIsSharpSourced && priceInBand && !stale;
...
function classifySharpPlay(row = {}, options = {}) {
  const {
    ...
    stale,
    consensusValidated
  } = resolveSharpPlaySignals(row, options);
  ...
  if (!sharpBookMovementConfirmed) {
    if (movementHistoryUsable !== true) passReasons.push('no_usable_line_history');
    if (!movementLabelSupported) passReasons.push(`movement_not_supportive_${row.movementLabel || 'unknown'}`);
    // consensusValidated rows pass on strong multi-book consensus + positive
    // edge even when the only movement source is the target book (no-vig
    // books like NoVigApp/Onyx are themselves the sharp reference). Don't
    // flag them as movement_source_is_target_book failures.
    if (requireIndependentSharpMovement && sourceIsTargetBook && !consensusValidated)
      passReasons.push('movement_source_is_target_book');
