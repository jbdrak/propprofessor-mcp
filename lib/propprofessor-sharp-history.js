...
function summarizeSharpMovement({ lineHistory, preferredBook, sharpBooks, options = /** @type {any} */ ({}) }) {
  ...
  const timestampsUsable = source.points.length >= 2 && source.points.every((point) => Number.isFinite(parseHistoryTimeMs(point?.time)));
  const lineHistoryUsable =
    !options.historyDegraded && Number(options.lineFieldMissingCount || 0) === 0 && timestampsUsable &&
