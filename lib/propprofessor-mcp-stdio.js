...
const AUTH_RECOVERY_HINT =
  'from your propprofessor-mcp install directory run PP_LOGIN_HEADLESS=false node scripts/pp-login.js (see README "Auth (one-time)" setup)';

function recoveryForCode(code, _category) {
  switch (code) {
    case 'AUTH_EXPIRED':
      return `Auth session expired. Recovery: ${AUTH_RECOVERY_HINT} (opens interactive Chromium — user must complete Google OAuth on app.propprofessor.com). Fallback: extract cookies from Chrome CDP and write to ~/.propprofessor/auth.json. After either path: rm ~/.propprofessor/token-cache.json to force fresh token fetch.`;
    case 'AUTH_REQUIRED':
      return `No auth file found. First-time setup: ${AUTH_RECOVERY_HINT} (opens interactive Chromium — user completes Google OAuth on app.propprofessor.com). Cookies saved to ~/.propprofessor/auth.json.`;
    case 'RATE_LIMITED':
    ...
    case 'INTERNAL_ERROR':
    default:
      return 'Check logs or file an issue at github.com/jbdrak/propprofessor-mcp';
  }
}
...
function categorizeError(error) {
  ...
  return createCategorizedError({
    ...
    status: error?.status,
    cause: error,
    recovery: error?.recovery || 'Check logs or file an issue at github.com/jbdrak/propprofessor-mcp'
  });
}
