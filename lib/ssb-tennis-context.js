...
const { parseRss, stripCdata } = require('./ssb-news-sources');
const _correctTennisTimes = require('./ssb-tennis');
const { lookupMatchTime, getCacheInfo } = require('./flashscore-times');

// Same pattern as ssb-news-sources.js: recreate promise each call
...
const SURFACE_PATTERNS = [
  ...
  { pattern: /\b(?:Genoa|Genova)\b/i, surface: 'Clay' },
  ...
  { pattern: /\b(?:Antalya|ATIK)\b/i, surface: 'Clay' },
...
/**
 * Guess context from one exact, fresh Flashscore match record.
 * @param {Object} match
 * @returns {string|null}
 */
function guessSurfaceFromFlashscoreMatch(match) {
  if (!match || typeof match !== 'object') return null;
  const explicit = String(match.surface || '')
    .trim()
    .toLowerCase();
  if (explicit) {
    if (explicit.includes('clay')) return 'Clay';
    if (explicit.includes('grass')) return 'Grass';
    if (explicit.includes('indoor')) return 'Indoor';
    if (explicit.includes('carpet')) return 'Carpet';
    if (explicit.includes('hard')) return 'Hardcourt';
  }
  return guessSurfaceFromTournament(match.tournament);
}

/**
 * Guess level from one exact, fresh Flashscore match record.
 * Generic ATP/WTA categories do not establish a level by themselves.
 * @param {Object} match
 * @returns {string|null}
 */
function guessLevelFromFlashscoreMatch(match) {
  if (!match || typeof match !== 'object') return null;
  const fromTournament = guessMatchLevel(match.tournament);
  if (fromTournament) return fromTournament;

  const category = String(match.category || '').toUpperCase();
  if (/GRAND\s*SLAM/.test(category)) return 'Grand Slam';
  if (/CHALLENGER/.test(category)) return 'Challenger';
  if (/ITF|FUTURES|WORLD\s+TENNIS\s+TOUR/.test(category)) return 'ITF Futures';
  return null;
}
...
function tourFromFlashscoreCategory(category) {
  const value = String(category || '').toUpperCase();
  if (/WOMEN|WTA/.test(value)) return 'wta';
  if (/MEN|ATP/.test(value)) return 'atp';
  return null;
}

/**
 * Resolve context from a fresh exact Flashscore pair.
 * @returns {Object|null}
 */
function resolveFlashscoreContext(player1, player2, start) {
  if (!player1 || !player2 || !start) return null;
  const cacheInfo = getCacheInfo();
  if (!cacheInfo || cacheInfo.fresh !== true) return null;

  const match = lookupMatchTime(player1, player2);
  if (!match || !match.tournament || !match.category) return null;
  if (dateKey(match.date) !== dateKey(start)) return null;

  const surface = guessSurfaceFromFlashscoreMatch(match);
  const level = guessLevelFromFlashscoreMatch(match);
  if (!surface && !level) return null;
  ...
  return {
    ...
    level,
    city: null,
    tour: tourFromFlashscoreCategory(match.category)
  };
}
...
async function searchTournamentFallback(player1, player2, _dateStr) {
  ...
  try {
    const { fetchEspnMatches, findEspnMatch } = require('./ssb-tennis');
    ...
    if (!match) return { surface: null, level: null };

    const eventText = [match.tournament, match.event, match.league].filter(Boolean).join(' ');
    if (!eventText) return { surface: null, level: null };

    const surface = guessSurfaceFromTournament(eventText);
    const level = guessMatchLevel(eventText);
    return { surface, level, tournament: eventText };
  } catch {
 ...
 * @param {string} [opts.start] - Scheduled start timestamp used to resolve a tournament from a matchup.
...
async function getTennisContext(opts = {}) {
  ...
  if (correctedStart && looksLikeMatchup(rawTournament)) {
    // Exact fresh Flashscore metadata outranks the broad weekly fallback.
    // During Grand Slam weeks, the weekly resolver can otherwise misclassify
    // a Challenger/ITF matchup as the Slam event.
    const flashscore = resolveFlashscoreContext(player1, player2, correctedStart);
    if (flashscore) {
      tournament = flashscore.tournament;
      resolvedFromMatchup = true;
      resolvedCity = flashscore.city;
      resolvedTour = flashscore.tour;
      resolvedSurface = flashscore.surface;
      resolvedLevel = flashscore.level;
    } else {
      ...
      if (resolved) {
        ...
        if (fb.surface || fb.level) {
          ...
          resolvedFromMatchup = true;
          tournament = fb.tournament || tournament;
        }
      }
    }
  }
...
module.exports = {
  ...
  guessMatchLevel,
  guessSurfaceFromFlashscoreMatch,
  guessLevelFromFlashscoreMatch,
  buildMatchupNewsUrl,
  parseRss,
  stripCdata,
  looksLikeMatchup,
