...
const { americanOddsToImpliedProbability, mapWithConcurrency } = require('./ssb-shared-utils');
...
/**
 * Translate a single-book tennis CLV into a canonical movement disposition.
 *
 * Tennis histories are single-book (no separate recent/full windows), so the
 * canonical computeMovementDisposition cannot be fed raw — it expects
 * movementGrade + movementLabel + direction windows. This adapter maps the
 * one available signal (clvProxyPct) into the exact fields computeMovementDisposition
 * reads, so tennis shares the SAME vocabulary and the SAME sharp-confirmation,
 * strong-consensus, and insufficient-history logic as every other sport.
 *
 * Mapping is intentionally aligned with the legacy inline tennis rule
 * (clv > 0.5 => supportive, clv < -0.5 => adverse) so public response shapes
 * and risk tiers do not shift:
 *   clv > 0.5   -> supportive label + yellow grade  -> supportive_bouncy
 *   clv < -0.5  -> adverse label + yellow grade    -> adverse_full
 *   otherwise   -> insufficient (flat / too-thin)   -> insufficient
 *
 * The translation is passed straight through computeMovementDisposition so the
 * single source of truth for the disposition vocabulary lives in
 * lib/ssb-movement-disposition.js.
 *
 * @param {number|null|undefined} clvProxyPct - Single-book CLV percentage.
 * @returns {string} Canonical disposition (supportive_bouncy | adverse_full | insufficient).
 */
function tennisMovementDisposition(clvProxyPct) {
  const clv = Number(clvProxyPct);
  if (!Number.isFinite(clv)) return 'insufficient';
  if (clv > 0.5) {
...
function normalizeTennisHistoryResponse(historyResponse) {
  if (Array.isArray(historyResponse)) return historyResponse;
  if (Array.isArray(historyResponse?.data)) return historyResponse.data;
  ...
  return Object.entries(historyResponse)
    .filter(([, entries]) => Array.isArray(entries))
    .flatMap(([book, entries]) => entries.map((entry) => ({ ...entry, book: entry?.book || book })));
}

function normalizeTennisHistoryItems(items) {
  return items
    .map((item) => ({ ...item, odds: item.odds ?? item.americanOdds }))
    .filter((item) => Number.isFinite(Number(item.odds)))
    .sort((a, b) => {
      const aTime = Number(a.start_ts ?? a.timestamp ?? a.time ?? 0);
      const bTime = Number(b.start_ts ?? b.timestamp ?? b.time ?? 0);
      return aTime - bTime;
    });
}
 ...
 * @param {Object} client - SSB API client with queryOddsHistory method.
...
async function enrichTennisEvCandidates(
  ...
  // hydrateScreenRowsWithHistory in lib/ssb-screen-history.js, so
    ...
    async (candidate) => {
      ...
      if (gameId && selectionId) {
        try {
          ...
          const historyItems = normalizeTennisHistoryItems(normalizeTennisHistoryResponse(historyResponse));

          if (historyItems.length >= 2) {
            lineHistory = historyItems;
            const first = lineHistory[0];
            const last = lineHistory[lineHistory.length - 1];
            openingOdds = Number(first.odds) || null;
            currentOdds = Number(last.odds) || null;
            ...
            if (Number.isFinite(openingProb) && Number.isFinite(currentProb)) {
              clvProxyPct = (currentProb - openingProb) * 100;
            }
          }
        } catch {
      ...
      const targetBookOdds = targetBookEntry ? Number(targetBookEntry.odds ?? targetBookEntry.noVigOdds) : null;
      ...
      // Tennis only has single-book history, so route the one CLV signal
      // through the canonical computeMovementDisposition producer via the
      // tennis CLV adapter (no competing inline threshold rule).
      const movementDisposition = tennisMovementDisposition(lineHistory.length >= 2 ? clvProxyPct : null);
      ...
      return {
        ...
        executionUnavailable: !Number.isFinite(targetBookOdds)
      };
    },
    { concurrency: 6 }
  );
  ...
  const ranked = rankScreenRows(enriched, {
    ...
    const clv = Number(row.clvProxyPct);
    const hasExecution = row.targetBookOdds != null && Number.isFinite(Number(row.targetBookOdds));
    const hasHistory = Array.isArray(row.lineHistory) && row.lineHistory.length >= 2;
    return {
      ...row,
      movementDisposition: tennisMovementDisposition(hasHistory ? clv : null),
      isActionable: hasExecution && hasHistory,
      executionUnavailable: !hasExecution,
