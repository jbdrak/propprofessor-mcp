...
const { normalizeDirection, normalizeMarketName, normalizeRow, parseHistoryTimeMs } = require('./ssb-shared-utils');
const { buildMatchupLabel, getSoccerEventIdentity } = require('./soccer-event-identity');
...
function extractScreenRows(payload, plays = []) {
  ...
  for (const rawRow of rows) {
    ...
    if (isNormalizedNonProp) {
      ...
      for (const book of books) {
        ...
        const common = {
          ...
          playType: row.market,
          market: row.market,
          game: buildMatchupLabel(row),
          gameId: row.gameId || row.id || null,
          league: row.league || row.sport || '',
          ...(/^(soccer|mls)$/i.test(String(row.league || row.sport || ''))
            ? { venueOrderVerified: getSoccerEventIdentity(row).venueOrderVerified }
            : {})
        };
    ...
    if (!rowExpanded && !candidateBooks.length) expanded.push(row);
  }
...
/**
 * Pick a retail proxy price (DraftKings preferred, FanDuel fallback) from the
 * per-book odds map that the ranker preserves as `allBookOdds`.
 *
 * This is PROXY metadata only: a readable retail line (DK/FD) shown next to the
 * ranked candidate so the user can eyeball the market, NOT an executable
 * Courtside-sharp quote. It must never overwrite `odds`/`book` (the executable
 * execution-book price the validator re-queries). Returns null when neither
 * proxy book has a parseable number in the map.
 *
 * @param {Object} [allBookOdds] - Map of book → odds (number or {odds1,odds2}).
 * @returns {{book:string, odds:number}|null}
 */
function selectProxyOdds(allBookOdds) {
  if (!allBookOdds || typeof allBookOdds !== 'object') return null;
  for (const book of ['DraftKings', 'FanDuel']) {
    if (!Object.prototype.hasOwnProperty.call(allBookOdds, book)) continue;
    const raw = allBookOdds[book];
    let odds = null;
    if (typeof raw === 'number') odds = raw;
    else if (raw && typeof raw === 'object') {
      // Per-side maps use { odds1, odds2 }; fall back to any finite number.
      odds = Number.isFinite(Number(raw.odds1))
        ? Number(raw.odds1)
        : Number.isFinite(Number(raw.odds2))
          ? Number(raw.odds2)
          : null;
    }
    if (Number.isFinite(odds)) return { book, odds };
  }
  return null;
}
