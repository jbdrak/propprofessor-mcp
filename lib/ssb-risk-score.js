...
const {
  ...
  GREEN_GATE,
  GREEN_GATE_MLB
} = require('./ssb-consensus-thresholds');
const { computeMovementDisposition } = require('./ssb-movement-disposition');

/**
 * Flatten the enriched row for the disposition module.
 *
 * computeMovementDisposition reads flat fields (row.movementGrade,
 * row.movementLabel, row.recentSharpMoveDirection, ...) but the enriched
 * row carries the direction data nested under `movementSummary`. Merge
 * summary up so the disposition module sees the same signal the ranked
 * rows see. `item` wins on collision — it holds the authoritative flat
 * grade/label when present.
 */
function dispositionOf(item = {}, gradeOverride) {
  // Trust an explicitly-set disposition when present (stub/sharp-play rows,
  // or rows the hydration layer already synthesized). Recompute only as a
  // fallback for rows missing the field — recomputing from partial fields
  // would read 'insufficient' for a row that honestly says supportive_clean.
  const ownDisposition = String(item.movementDisposition || item.row?.movementDisposition || '')
    .trim()
    .toLowerCase();
  const KNOWN = ['supportive_clean', 'supportive_bouncy', 'adverse_recent', 'adverse_full', 'insufficient'];
  if (KNOWN.includes(ownDisposition)) return ownDisposition;

  // computeMovementDisposition reads flat fields (row.movementGrade,
  // row.movementLabel, row.recentSharpMoveDirection, ...) but the enriched
  // row carries the direction data nested under `movementSummary`. Merge
  // summary up so the disposition module sees the same signal the ranked
  // rows see. Own-props on `item` win — EXCEPT undefined ones, which would
  // clobber a real summary value (e.g. item.movementLabel: undefined
  // wiping movementSummary.movementLabel: 'supportive'). Skip undefined
  // own-props so a present summary value survives.
  const flat = {};
  for (const [key, value] of Object.entries(item)) {
    if (value !== undefined) flat[key] = value;
  }
  if (gradeOverride !== undefined) flat.movementGrade = gradeOverride;
  return computeMovementDisposition({ ...(item.movementSummary || {}), ...flat });
}
...
function gradeMovementQuality(item = {}) {
  ...
  // multiWindowScore is null when there's no line history — don't punish
  // absence of data. The previous `?? 0` collapsed MISSING multi-window data
  // into a score of 0, which then failed the `>= 0.66` sustained-agreement
  // check and silently blocked GREEN (→ yellow → TIER 3) on every row whose
  // feed lacked multi-window history (common in MLB/UFC). Missing is "no
  // data", not "no agreement".
  const multiWindowInsufficientData = Boolean(item.multiWindowInsufficientData);
  const rawMultiWindowScore = item.multiWindowScore ?? item.movementSummary?.multiWindowScore;
  const multiWindowScore =
    multiWindowInsufficientData || rawMultiWindowScore === undefined || rawMultiWindowScore === null
      ? null
      : Number(rawMultiWindowScore);
  ...
  // Execution quality is a hard playability gate. A row with no reliable
  // executable quote must never be graded as an actionable signal, even when
  // movement or consensus inputs look strong.
  if (badExecution) {
    return 'red';
  }
...
function calculateRiskScore(item = {}) {
  ...
  const consensusBookCount = Number(item.consensusBookCount || Object.keys(item.allBookOdds || {}).length || 0);
  const steamMove = Boolean(item.steamMove);
  const steamOriginatorCount = Number(item.steamOriginatorCount || 0);
  const clvPct = Number(item.clvProxyPct ?? item.clv ?? 0);
  ...
  // Weight by provenance: a move confirmed by sharp ORIGINATORS (Pinnacle/
  // Circa/BookMaker/BetOnline move first on informed action) is the real
  // signal; a steam of retail FOLLOWERS (DraftKings/FanDuel/BetMGM) copying
  // the line afterward is weaker. Full -2 only when >=1 originator moved;
  // followers-only steam gets a milder -1 so we don't overstate a copied line.
  if (steamMove && movementLabel === 'supportive') score -= steamOriginatorCount >= 1 ? 2 : 1;
...
function getKaiCall(item = {}) {
  ...
  // Insufficient-movement guard (2026-08-15): the disposition module is the
  // single source of truth for "can't evaluate". When it returns
  // 'insufficient' (no direction data below the consensus floor, or a stale
  // supportive quote older than 10 min), the verdict must never read BET —
  // previously the grade path trusted the backend's green stamp and stamped
  // BET on rows whose honest movement read insufficient. Playable thin-history
  // rows were already upgraded to supportive_bouncy by the disposition module
  // (sharp confirm or 5+ books with non-negative CLV), so a residual
  // 'insufficient' genuinely means not enough data. PASS per the movement
  // disposition contract.
  //
  // The derived grade is passed in because the enriched row's flat
  // movementGrade is not yet set at rank time — the disposition module
  // reading a missing grade would fall through to 'insufficient' for rows
  // that legitimately grade green (steam-exempt) and misfire the guard.
  if (dispositionOf(item, grade) === 'insufficient') {
    return 'PASS';
  }
...
function getConfidenceTier(item = {}) {
  ...
  // Insufficient-movement guard (2026-08-15): mirror getKaiCall — a row the
  // disposition module reads as 'insufficient' must never carry a BET-tier.
  // PASS verdicts always force TIER 4 (see handlers.js), so cap here too or
  // the tier and verdict contradict each other. Derived grade passed in to
  // avoid the rank-time missing-grade fallthrough (see getKaiCall).
  if (dispositionOf(item, grade) === 'insufficient') {
    return 'TIER 4';
  }
...
function buildRationale(item = {}) {
  ...
  // Surface which kind so the rationale reflects the real signal strength.
  const steamBooks = Number(item.steamBookCount || 0);
  const steamOriginators = Number(item.steamOriginatorCount || 0);
  if (item.steamMove && steamBooks > 0) {
    const originatorNote = steamOriginators > 0 ? `, ${steamOriginators} originator` : ', followers-only';
    parts.push(`steam (${steamBooks} books${originatorNote})`);
  }
 ...
 */
function suggestStakes(opts = /** @type {{ bankroll?: number, plays?: any[] }} */ ({})) {
  const { bankroll = 1000, plays = [] } = opts;
