...
function pendingResult(reason) {
  return { status: 'pending', outcome: null, result: null, isSettled: false, reason };
}
...
function solve(ledger, opts = {}) {
  ...
  for (const bet of bets) {
    ...
    if (!betKey) {
      skipped.push({ betId: null, reason: 'bet has no id; settlement rows require a bet id (official bets always carry one)' });
      continue;
    }
