...
const { normalizeSelectionId } = require('./ssb-api');
const { resolveHistoryForEntity, normalizeHistoryPayload, getOddsHistoryStartTimestamp } = require('./ssb-history');
...
// /odds_history_new calls could pile onto a throttled upstream. Requests are
// also paced through one shared gate because concurrency alone still creates
// a burst when each request is short-lived.
const DEFAULT_HISTORY_CONCURRENCY = 3;
const DEFAULT_HISTORY_MIN_INTERVAL_MS = 50;
const DEFAULT_HISTORY_THROTTLE_COOLDOWN_MS = 60000;
const historyGateStates = new WeakMap();

function getHistoryGateState(client) {
  let state = historyGateStates.get(client);
  if (!state) {
    state = {
      nextRequestAt: 0,
      haltedUntil: 0,
    ...
    historyGateStates.set(client, state);
  }
...
async function resolveScreenHistoryRow(
  row,
  {
    sourceRows,
    cache,
    ...
    enableLineFallback,
    sharpOddsProvider,
    preferSharpOddsHistory,
    queryHistoryWithGate,
    gateState
  }
) {
  ...
  if (!cache.has(cacheKey)) {
    cache.set(
      ...
      resolveHistoryWithLineFallback({
        ...
        sharpOddsProvider,
        preferSharpOddsHistory,
        queryHistoryFn: queryHistoryWithGate,
        onHistoryError: (error) => {
          if (!isThrottleOrCircuitError(error)) return true;
          const now = Date.now();
          if (now - gateState.lastThrottleLogAt < 10000) return false;
          gateState.lastThrottleLogAt = now;
          return true;
        }
      })
    );
  ...
  if (resolved?.lineHistoryAvailable) {
    return {
      ...
      historySportsbooksRequested: Array.isArray(resolved.historySportsbooksRequested)
        ...
        : [],
      historyProvider: resolved.historyProvider || undefined,
      historyReason: resolved.historyReason || undefined,
      historyWarning: resolved.historyWarning || undefined,
      movementSourceBook: resolved.movementSourceBook || undefined,
      movementMode: resolved.movementMode || undefined,
      ...
      // Price history can qualify movement; lineHistoryUsable stays false
      // when historical line fields are unavailable.
      priceHistoryUsable: resolved.priceHistoryUsable === true,
      priceHistoryScope: typeof resolved.priceHistoryScope === 'string' ? resolved.priceHistoryScope : null,
      priceHistorySource: typeof resolved.priceHistorySource === 'string' ? resolved.priceHistorySource : null,
      priceHistoryPointCount: Number.isFinite(Number(resolved.priceHistoryPointCount))
        ? Number(resolved.priceHistoryPointCount)
        : 0
    };
  }
  ...
  return {
    ...
    ...(resolved && typeof resolved === 'object' && 'historyError' in resolved
      ...
      : {}),
    ...(resolved && typeof resolved === 'object' && 'historyProvider' in resolved && resolved.historyProvider
      ? { historyProvider: resolved.historyProvider }
      : {}),
    ...(resolved && typeof resolved === 'object' && 'historyWarning' in resolved && resolved.historyWarning
      ? { historyWarning: resolved.historyWarning }
      : {}),
    ...(resolved && typeof resolved === 'object' && 'historyReason' in resolved && resolved.historyReason
      ? { historyReason: resolved.historyReason }
      : {}),
    ...
    ...(resolved && typeof resolved === 'object' && 'historyErrorStatus' in resolved
      ...
      : {}),
    // Native PP selection-scoped price-history provenance passthrough on the
    // degraded path. lineHistoryAvailable stays false here.
    priceHistoryUsable: Boolean(resolved && resolved.priceHistoryUsable === true),
    priceHistoryScope: resolved && typeof resolved.priceHistoryScope === 'string' ? resolved.priceHistoryScope : null,
    priceHistorySource:
      resolved && typeof resolved.priceHistorySource === 'string' ? resolved.priceHistorySource : null,
    priceHistoryPointCount:
      resolved && Number.isFinite(Number(resolved.priceHistoryPointCount)) ? Number(resolved.priceHistoryPointCount) : 0
  };
}
 ...
 * @param {string[]}      [options.sharpOddsBooks=[]] - SharpOdds-only movement books.
 * @param {number}        [options.concurrency=3] - Max number of concurrent history resolutions.
 * @param {number}        [options.minIntervalMs=50] - Minimum spacing between history requests.
 * @param {number}        [options.throttleCooldownMs=60000] - How long to stop history requests after a throttle.
 * @param {boolean}       [options.enableLineFallback=true] - Try adjacent line variants when exact history is empty.
...
async function hydrateScreenRowsWithHistory(
  rows,
  opts = /** @type {{ client?: any, lookbackHours?: number, historySportsbooks?: string[], preferredBook?: string, sharpBooks?: string[], sharpOddsBooks?: string[], concurrency?: number, minIntervalMs?: number, throttleCooldownMs?: number, enableLineFallback?: boolean, sharpOddsProvider?: any, preferSharpOddsHistory?: boolean }} */ ({})
) {
  const {
    client,
    ...
    minIntervalMs = DEFAULT_HISTORY_MIN_INTERVAL_MS,
    throttleCooldownMs = DEFAULT_HISTORY_THROTTLE_COOLDOWN_MS,
    enableLineFallback = true,
    sharpOddsProvider = null,
    preferSharpOddsHistory = false
  } = opts;
  ...
  const cache = new Map();
  const gateState = getHistoryGateState(client);

  async function queryHistoryWithGate(params) {
    if (gateState.haltedError) throw gateState.haltedError;
    const interval = Math.max(0, Number(minIntervalMs) || 0);
    const now = Date.now();
    const waitMs = Math.max(0, gateState.nextRequestAt + interval - now);
    gateState.nextRequestAt = now + waitMs;
    if (waitMs > 0) await new Promise((resolve) => setTimeout(resolve, waitMs));
    if (gateState.haltedError) throw gateState.haltedError;
    try {
      return await client.queryOddsHistory(params);
    } catch (error) {
      if (isUpstreamThrottleError(error)) {
        gateState.haltedError = error;
        gateState.haltedUntil = Date.now() + Math.max(0, Number(throttleCooldownMs) || 0);
      }
      throw error;
    }
  }
  ...
  async function runWorker() {
    while (nextIndex < sourceRows.length) {
      ...
      nextIndex += 1;
      results[currentIndex] = await resolveScreenHistoryRow(sourceRows[currentIndex], {
        sourceRows,
        cache,
        client,
        lookbackHours,
...
module.exports = {
  ...
  // Exported so the handler factory (scripts/server/handlers.js) can keep its
  // default in lock-step with the hydration gate's production pacing. Tests
  // pin this at 50 to guarantee the default is never silently relaxed.
 ...
 * by lib/ssb-api.js (tagged HTTP errors carrying a numeric `status`)
 * and lib/ssb-circuit-breaker.js (CircuitBreakerOpenError with
 * `name`/`code`). Includes the LOCAL budget-exhaustion error: it must fail
 * soft (degraded row) and suppress variant amplification, but it never trips
 * the persistent per-client halt (see isUpstreamThrottleError).
 *
 * @param {*} error - Error thrown by the history query path.
 * @returns {boolean} True for HTTP 429 throttle errors, circuit-open errors,
 *   and local budget exhaustion.
 */
function isThrottleOrCircuitError(error) {
  ...
  return (
    ...
    error.code === 'ODDS_HISTORY_BUDGET_EXHAUSTED'
  );
}

/**
 * Upstream-only throttle errors (429 / circuit-open). Only these trip the
 * persistent per-client halt — a local budget-exhaustion error is a
 * per-window safety throttle, not a server signal, so it must never poison
 * later hydration calls from a different scan context.
 *
 * @param {*} error - Error thrown by the history query path.
 * @returns {boolean} True only for HTTP 429 and circuit-open errors.
 */
...
async function resolveHistoryWithLineFallback({
  ...
  let sharpError = null;
  if (sharpOddsProvider && typeof sharpOddsProvider.resolve === 'function') {
    try {
      sharpResult = await sharpOddsProvider.resolve(target, {
        preferredBook,
        sharpBooks: Array.isArray(sharpOddsBooks) && sharpOddsBooks.length ? sharpOddsBooks : sharpBooks
      });
      if (sharpResult?.lineHistoryAvailable === true) return sharpResult;
    } catch (error) {
  ...
  const exact = await resolveHistoryForEntity({
    ...
    if (!onHistoryError || onHistoryError(error) !== false) {
      process.stderr.write(`[ssb-for-agents] History resolution failed: ${error?.message || error}\n`);
    }
    const degraded = {
      lineHistory: [],
      lineHistoryAvailable: false,
      ...
      priceHistoryUsable: false,
      priceHistoryScope: null,
      priceHistorySource: null,
      priceHistoryPointCount: 0
    };
  ...
  const exactMetadata = /** @type {any} */ (exact);
  const exactHistoryDegraded =
    exactMetadata &&
    typeof exactMetadata === 'object' &&
    (exactMetadata.historyDegraded === true || Number(exactMetadata.lineFieldMissingCount || 0) > 0);
  if (exact.lineHistoryAvailable && !exactHistoryDegraded) return exact;

  // Preserve the PP fallback result when it is degraded, but retain the
  // SharpOdds diagnostic that explains why the primary source was unusable.
  if (preferSharpOddsHistory && (sharpResult || sharpError)) {
    return {
      ...exact,
      ...(sharpResult?.historyProvider ? { historyProvider: sharpResult.historyProvider } : {}),
      ...(sharpResult?.historyReason ? { historyReason: sharpResult.historyReason } : {}),
      ...(sharpResult?.historyWarning ? { historyWarning: sharpResult.historyWarning } : {}),
      ...(sharpError
        ? {
            historyProvider: 'sharpodds',
            historyReason: 'provider_error',
            historyWarning: sharpError?.message || String(sharpError)
          }
        : {})
    };
  }

  if (!enableLineFallback) return exact;
  ...
  for (const newLine of variants) {
    ...
    if (variantHistory.length >= 2) {
      return {
        ...
        priceHistoryScope: null,
        priceHistorySource: null,
        priceHistoryPointCount: variantHistory.filter(
          (entry) => Number.isFinite(Number(entry?.odds)) && entry?.time !== null && entry?.time !== undefined
        ).length
      };
    }
  }
