...
const { normalizeText, parseHistoryTimeMs, getOddsHistoryStartTimestamp } = require('./ssb-shared-utils');
 ...
 *   priceHistoryUsable: boolean, priceHistoryScope: string|null,
 *   priceHistorySource: string|null, priceHistoryPointCount: number}>}
...
async function resolveHistoryForEntity({
  ...
  const priceHistoryPointCount = rawLineHistory.filter(
    (entry) => Number.isFinite(Number(entry?.odds)) && Number.isFinite(parseHistoryTimeMs(entry?.time))
  ).length;
  ...
  const normalizedTargetSelectionId = normalizeSelectionId(
    target?.selectionId ?? target?.selection_id ?? target?.selectionID ?? target?.selection ?? null
  );
  const isExactSelectionPriceUsable =
    Boolean(
      normalizedTargetSelectionId &&
      selectionId &&
      normalizedTargetSelectionId === selectionId &&
      matchStrength &&
      // @ts-expect-error
      matchStrength.selectionIdMatch === true
    ) && priceHistoryPointCount >= 2;
  ...
  return {
    ...
    // Native PP selection-scoped price-history provenance: usable only when
    // the resolved history matched the exact target selectionId and the
    // upstream returned >=2 real timestamped odds points (numeric odds +
    // parseable time). Line semantics above are preserved exactly.
    priceHistoryUsable: isExactSelectionPriceUsable,
    priceHistoryScope: isExactSelectionPriceUsable ? 'selection_id' : null,
    priceHistorySource: isExactSelectionPriceUsable ? 'odds_history' : null,
    priceHistoryPointCount
  };
}
