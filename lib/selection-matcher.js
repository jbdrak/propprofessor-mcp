 ...
 * @param {{ league?: string, market?: string, selection?: string, gameId?: string, playId?: string, requestedBook?: string }} [opts]
 * @returns {Object|null} The uniquely-matching row, or null.
 */
function findBestMatchGameIdChanged(detailRows = [], opts = {}) {
  const { league = '', market = '', selection = '', gameId = '', playId = '', requestedBook = '' } = opts;
  ...
  const candidates = detailRows.filter((r) => {
    ...
    if (!rowDate || rowDate !== identity.dateKey) return false;
    // Cross-check the row's own gameId timestamp when it parses.
    const rowIdentity = parseGameIdIdentity(r.gameId);
    if (rowIdentity && rowIdentity.dateKey && rowIdentity.dateKey !== identity.dateKey) return false;
    return true;
  });
  ...
  const distinctGameIds = new Set(
    candidates.map((r) => String(r.gameId || '').trim()).filter(Boolean)
  );
  ...
  const byKey = candidates.filter((r) => {
    const rowKey =
      identityKey(r.selectionKey) ||
      identityKey(r.selection || r.participant || '');
