...
function buildRankedIntermediateRow(
  ...
  const executionQuality = String(row?.executionQuality ?? 'unknown').trim().toLowerCase();
  ...
  return {
    ...
    gateReason: gate.passed ? gate.reason : !preferredBookMatch ? `no price for ${preferredBook || 'preferred book'}` : `execution ${executionQuality}`,
