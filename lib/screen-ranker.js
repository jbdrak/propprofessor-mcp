...
/**
 * Totals dedup: per game, keep only the best totals play (highest consensus edge).
 * Prevents both Over and Under from appearing against the same game — they can't
 * both win, and showing both as TIER 1 is contradictory and noisy.
 *
 * Unlike side markets (where we resolve by team direction), totals lines vary
 * by number (O184.5, U177.5), so the conflict is any two totals on the same game.
 * We keep the highest-edge play and downgrade the rest by one tier.
 */
...
function resolveTotalsConflicts(ranked, { debug = false } = {}) {
  ...
  for (const [gameId, rows] of gameGroups.entries()) {
    ...
    // Keep the best (highest consensus edge), downgrade rest
...
function resolveAlternateLines(ranked, { debug = false } = {}) {
  ...
  for (const row of ranked) {
    ...
    const isSpread = market === 'point spread' || market === 'run line' || market === 'game handicap' || market === 'spread';
    ...
    if (!current || row.consensusBookCount > current.books) {
      mainLines.set(key, {
        lineNumber,
        books: row.consensusBookCount || 0,
...
function rankScreenRows(
  ...
  const filterPipelineResult = expanded
    .map((row) => { // eslint-disable-line complexity
