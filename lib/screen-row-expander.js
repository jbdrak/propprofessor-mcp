 ...
 */
// @ts-expect-error
function expandExtractedScreenRow(row, { preferredBook, requirePreferredBook }) {
...
function expandLegacyScreenRow(row, { preferredBook, requirePreferredBook }) {
...
function expandScreenRow(row, { preferredBook = 'NoVigApp', requirePreferredBook = false } = {}) {
  ...
  if (row?.selections && (row?.book || row?.sportsbook)) {
    return expandExtractedScreenRow(row, { preferredBook, requirePreferredBook });
  }

  return expandLegacyScreenRow(row, { preferredBook, requirePreferredBook });
}
