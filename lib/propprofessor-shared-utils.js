 ...
 *
 * The PropProfessor /screen endpoint returns `start` in mixed formats:
 ...
 *
 * Wraps `worker(item)` in a Promise.race against a timer. On timeout the
 * worker is NOT cancelled (no AbortController seam exists in the screen
 * pipeline), but the fan-out resolves with a structured timeout error instead
 * of leaving the pair's slot forever empty — the caller records the failure
 * and continues, so sibling leagues keep their rows.
 *
...
function withPairTimeout(worker, timeoutMs, { label = '' } = {}) {
  ...
  return async (item, index) => {
    ...
    try {
      return await Promise.race([worker(item, index), timeout]);
    } finally {
      clearTimeout(timer);
    }
  };
...
function createCrossCallMemoizedQuery(
  fn,
  opts = /** @type {{ cache: import('./propprofessor-lru-cache').LruCache, keyFn?: (params: any) => string }} */ ({})
) {
