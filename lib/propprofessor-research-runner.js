...
async function runResearchOnTopRows({
  ...
  const results = await mapWithConcurrency(
    ...
    async (row) => {
      ...
      // Tennis Moneyline is the exception: the Elo shadow model is Moneyline-only
      // and lives on the tennis game context, so a bare player-name Tennis
      // Moneyline selection MUST go to game context or Elo can never fire for
      // the market it targets (quick_screen / recommended_bets tennis plays).
      const isTennisMoneyline = league.toLowerCase() === 'tennis' && /^(moneyline|money line|ml)$/i.test(market.trim());
      const isPlayer = !isTennisMoneyline && isPlayerSelection(player);
      ...
      try {
        ...
        // Elo is additive game-context-only context (Tennis shadow model):
        // game-context results carry it verbatim; player contexts and error
        // rows never do. It must not change riskFlag/riskSummary.
        if (!isPlayer && ctx.elo) {
          result.elo = ctx.elo;
        }
