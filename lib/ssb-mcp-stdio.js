 ...
 *   'Unexpected SSB MCP error' when omitted.
...
function createCategorizedError({
  ...
  const error =
    /** @type {Error & { code?: string, category?: string, status?: number, cause?: unknown, recovery?: string }} */ (
      new Error(message || 'Unexpected SSB MCP error')
    );
  error.code = code;
...
/**
 * Portable auth recovery hint. Points users at the package-relative login script
 * (documented in README "Auth (one-time)" setup) instead of author-machine
 * absolute paths or private per-user scripts, so it works from any install.
 */
const AUTH_RECOVERY_HINT =
  'from your ssb-for-agents install directory run PP_LOGIN_HEADLESS=false node scripts/pp-login.js (see README "Auth (one-time)" setup)';

function recoveryForCode(code, _category) {
  switch (code) {
    case 'AUTH_EXPIRED':
      return `Auth session expired. Recovery: ${AUTH_RECOVERY_HINT} (opens interactive Chromium — user must complete Google OAuth on app.propprofessor.com). Fallback: extract cookies from Chrome CDP and write to ~/.ssb-for-agents/auth.json. After either path: rm ~/.ssb-for-agents/token-cache.json to force fresh token fetch.`;
    case 'AUTH_REQUIRED':
      return `No auth file found. First-time setup: ${AUTH_RECOVERY_HINT} (opens interactive Chromium — user completes Google OAuth on app.propprofessor.com). Cookies saved to ~/.ssb-for-agents/auth.json.`;
    case 'RATE_LIMITED':
      return 'Wait 60 seconds and retry';
    case 'TOKEN_REFRESH_FAILED_BOTH_PATHS':
      return `Auth token expired. Recovery: ${AUTH_RECOVERY_HINT}.`;
    case 'BACKEND_DOWN':
      return 'Try again in a few minutes';
    case 'BACKEND_ERROR':
      return 'Check SSB status or try again later';
    ...
    case 'INTERNAL_ERROR':
    default:
      return 'Check logs or file an issue at github.com/jbdrak/ssb-for-agents';
  }
}

/**
 * Classify a raw error into a structured SSB error with code, category,
...
function categorizeError(error) {
  ...
  const message = String(error?.message || error || 'Unexpected SSB MCP error');
  ...
  if (error?.code === 'TOKEN_REFRESH_FAILED_BOTH_PATHS' || lower.includes('token refresh failed both paths')) {
    ...
    return createCategorizedError({
      ...
      recovery:
        error?.recovery ||
        (isRateLimited ? 'Wait 60 seconds and retry' : `Auth token expired. Recovery: ${AUTH_RECOVERY_HINT}.`)
    });
  }
  ...
  if (error?.status >= 500 || lower.includes('backend')) {
    return createCategorizedError({
      ...
      status: error?.status,
      cause: error,
      recovery: error?.recovery || 'Check SSB status or try again later'
    });
  }
  ...
  return createCategorizedError({
    ...
    status: error?.status,
    cause: error,
    recovery: error?.recovery || 'Check logs or file an issue at github.com/jbdrak/ssb-for-agents'
  });
}
 ...
 * for debugging (controlled by the `SSB_MCP_NDJSON` env var).
 *
 ...
 *   JSON object per line). Defaults to reading `SSB_MCP_NDJSON` env var.
 * @returns {string} The encoded message string.
 */
function encodeMessage(payload, { newlineJson = process.env.SSB_MCP_NDJSON === 'true' } = {}) {
 ...
 *   Defaults to reading `SSB_MCP_NDJSON` or
 *   `SSB_MCP_DEBUG_NDJSON` env vars.
...
function createStdioMessageReader(
  onMessage,
  { allowNewlineJson = process.env.SSB_MCP_NDJSON === 'true' || process.env.SSB_MCP_DEBUG_NDJSON === 'true' } = {}
) {
  let buffer = '';
  ...
  return function onData(chunk) {
    ...
    while (buffer.length > 0) {
        ...
        if (allowNewlineJson) {
          ...
          if (line) {
            try {
              ...
              process.stderr.write(`[ssb-for-agents] Skipping malformed NDJSON line: ${line.slice(0, 200)}\n`);
            }
          }
          continue;
        }
      ...
      if (!contentLengthLine) {
        ...
        process.stderr.write('[ssb-for-agents] Skipping frame with missing Content-Length header\n');
        continue;
      }
      ...
      if (!Number.isFinite(contentLength) || contentLength <= 0) {
        ...
        process.stderr.write(
          `[ssb-for-agents] Skipping frame with invalid Content-Length header: ${contentLengthLine}\n`
        );
        continue;
      }
      ...
      try {
        ...
        process.stderr.write(`[ssb-for-agents] Skipping frame with malformed JSON body: ${body.slice(0, 200)}\n`);
      }
    }
  };
}
