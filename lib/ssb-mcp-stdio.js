...
function recoveryForCode(code, _category) {
  switch (code) {
    case 'AUTH_EXPIRED':
      return `Auth session expired. Recovery: ${AUTH_RECOVERY_HINT} (opens interactive Chromium — user must complete Google OAuth on app.propprofessor.com). Fallback: extract cookies from Chrome CDP and write to ~/.ssb/auth.json. After either path: rm ~/.ssb/token-cache.json to force fresh token fetch.`;
    case 'AUTH_REQUIRED':
      return `No auth file found. First-time setup: ${AUTH_RECOVERY_HINT} (opens interactive Chromium — user completes Google OAuth on app.propprofessor.com). Cookies saved to ~/.ssb/auth.json.`;
    case 'RATE_LIMITED':
