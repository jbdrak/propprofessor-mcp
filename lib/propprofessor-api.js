...
// Local safety window: the 75-call budget applies per rolling window. When
...
const ODDS_HISTORY_REQUEST_BUDGET = 75;
...
async function acquireOddsHistorySlot(state) {
  ...
  const start = state.startTail.then(async () => {
    ...
    state.nextRequestAt = Date.now() + ODDS_HISTORY_MIN_INTERVAL_MS;
...
async function runOddsHistoryRequest(gateKey, requestFn) {
  const state = getOddsHistoryGateState(gateKey);
  await acquireOddsHistorySlot(state);
  try {
    try {
      ...
      if (Number(error?.status) === 429) {
        ...
        // slot — otherwise circuit-open churn would burn the 75-call window
...
function createPropProfessorClient({
  ...
  return {
    ...
    queryOddsHistory(opts = {}) {
      ...
      const request = runOddsHistoryRequest(authFile, () =>
