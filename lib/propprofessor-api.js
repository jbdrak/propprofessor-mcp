...
function createPropProfessorClient({
  ...
  const getTrpcJSON = makeTrpcJSONFetcher({ fetchImpl, requestTimeoutMs, retryDelaysMs, getAccessToken, invalidateAccessToken });

  const ctx = { requestJSON, retryDelaysMs, getTrpcJSON, now, authFile, fetchImpl, getAccessToken, counters, effectiveOddsHistoryMinIntervalMs };
