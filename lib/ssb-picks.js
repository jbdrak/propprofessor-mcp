...
const PICKS_DIR = path.join(require('node:os').homedir(), '.ssb-for-agents');
  ...
  try {
    ...
    if (version > CURRENT_PICKS_SCHEMA_VERSION && !warnedAboutLegacySchema) {
      process.stderr.write(
        `[ssb-picks] WARNING: picks.json was written by a newer version of the MCP server ` +
    ...
    if (version === 0 && !warnedAboutLegacySchema) {
      ...
      process.stderr.write(
        `[ssb-picks] INFO: picks.json has no schemaVersion — treating as v0. ` +
...
function resolvePick(id, result) {
  ...
  try {
    const { recordResolution } = require('./ssb-signal-calibration');
