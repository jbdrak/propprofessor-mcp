...
function buildEvCandidatesTool() {
  return {
    name: 'ev_candidates',
    description:
      'Query the Positive EV board and return candidate plays for enabled books. This is the primary discovery source; exact validation still controls actionability. NOTE: tier/kaiCall/edge/screenScore are signal-quality ratings (what sharp books are doing), NOT win-probability predictions.',
    inputSchema: {
      type: 'object',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        selection: {
          type: 'string',
          description:
            'Optional exact selection/line filter, e.g. "Over 22.5" or "Celtics". When provided, only the top-level row(s) whose selection matches (case-insensitive) are returned, with nested selections preserved. Fails closed with matchedRows 0 + selectionNotFound metadata when no row matches.'
        },
        books: {
          type: 'array',
          items: { type: 'string' },
...
function buildValidationTools() {
  return [
    buildEvCandidatesTool(),
    buildPlayDetailsTool(),
    {
      name: 'validate_play',
      description:
      ...
      inputSchema: {
        ...
        properties: {
          ...
          playId: {
            type: 'string',
            description:
              'Canonical playId from a prior quick_screen or screen_ranked row. When provided, row matching uses exact playId lookup and skips fragile string comparison — pass this to avoid "no row matched" lookup_failed errors. Takes priority over selection string matching.'
          },
          market: {
            type: 'string',
            description:
