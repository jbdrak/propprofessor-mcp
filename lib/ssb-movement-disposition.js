...
const { MOVEMENT_DISPOSITION_SUPPORT } = require('./ssb-consensus-thresholds');
...
// movement point. It does not describe the age of the executable quote itself.
const STALE_MOVEMENT_HISTORY_MAX_AGE_MS = 10 * 60 * 1000;

function hasCurrentQuote(row) {
  return [row?.odds, row?.currentOdds, row?.targetBookOdds].some(
    (value) => value !== null && value !== undefined && value !== '' && Number.isFinite(Number(value))
  );
}

function isMovementEvidenceAged(row) {
  const ageMs = Number(row?.lastPointAgeMs);
  return Number.isFinite(ageMs) && ageMs > STALE_MOVEMENT_HISTORY_MAX_AGE_MS;
}

function computeMovementDisposition(row) {
  if (!row || typeof row !== 'object') return 'insufficient';
  const movementHistoryUsable = row.movementHistoryUsable ?? row.priceHistoryUsable ?? row.lineHistoryUsable;
  if (movementHistoryUsable === false && row.priceHistoryUsable !== true) return 'insufficient';

  // Movement-history age qualifies the movement claim, not the current quote.
  // A current exact quote is authoritative for execution. If it is absent,
  // preserve the fail-closed stale behavior; corroborated support retains its
  // existing exception below.
  const disposition = computeMovementDispositionUnchecked(row);
  if (isMovementEvidenceAged(row) && (disposition.startsWith('supportive_') || disposition.startsWith('adverse'))) {
    if (hasCurrentQuote(row)) return disposition;
