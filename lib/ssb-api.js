...
const { getSharpBookComparisonSet, ALL_SCREEN_BOOKS, uniqueBooks } = require('./ssb-sharp-books');
const {
...
const { getOddsHistoryStartTimestamp, DEFAULT_LEAGUES } = require('./ssb-shared-utils');
const { getOrCreateBreaker, CircuitBreakerOpenError } = require('./ssb-circuit-breaker');
const {
  isScopeAborted,
...
const BACKEND_EV_PROF_URL = 'https://backend.propprofessor.com/ev-prof';
...
// History gate concurrency: the gate was serial (1) which capped the whole
// scan at ~10 history calls/sec no matter the budget. Upstream 429s still
// halt the gate with cooldown (see runOddsHistoryRequest), so raising this
// only spends the local budget faster — it cannot amplify a real outage.
// Env-tunable (PP_ODDS_HISTORY_CONCURRENCY, clamped 1..8), default 3.
const ODDS_HISTORY_MAX_CONCURRENCY = readOddsHistoryConcurrency();
function readOddsHistoryConcurrency() {
  const raw = process.env.PP_ODDS_HISTORY_CONCURRENCY;
  const parsed = Number(raw);
  if (Number.isFinite(parsed)) return Math.max(1, Math.min(8, Math.floor(parsed)));
  return 3;
}
...
const getOddsHistoryInflight = createScopedInflight();
...
async function acquireOddsHistorySlot(state, minIntervalMs, maxConcurrency = ODDS_HISTORY_MAX_CONCURRENCY) {
  const concurrency = Math.max(1, Math.floor(Number(maxConcurrency) || ODDS_HISTORY_MAX_CONCURRENCY));
  if (state.active < concurrency) {
    ...
    try {
      await waitForScope(pending);
    } catch (error) {
      if (granted) releaseOddsHistorySlot(state);
      else {
...
async function runOddsHistoryRequest(
  gateKey,
  requestFn,
  { minIntervalMs = ODDS_HISTORY_MIN_INTERVAL_MS, maxConcurrency = ODDS_HISTORY_MAX_CONCURRENCY } = {}
) {
  // Cooperative scan-deadline cancellation: do not claim a budget slot, wait
  // in the queue, or open a network request once the owning pair's scope is
  // aborted. Check BEFORE the queue wait / budget claim so a finished scan
  // cannot enqueue fresh odds-history work. Fail-closed and non-retryable;
  // sibling scopes are untouched because each pair gets its own scope id.
  if (isScopeAborted()) {
    throw new ScopeAbortError('odds history not started: scan scope aborted');
  }
  const state = getOddsHistoryGateState(gateKey);
  await acquireOddsHistorySlot(state, minIntervalMs, maxConcurrency);
  try {
    try {
...
function createTaggedError({ message, code, category, status, retryable, details } = {}) {
  const error =
    /** @type {Error & { code?: string, category?: string, status?: number, retryable?: boolean, details?: unknown }} */ (
      new Error(message || 'SSB request failed')
    );
...
/**
 * Create a tagged timeout error with SSB-specific metadata.
...
function createTimeoutError({ source, timeoutMs, cause } = {}) {
  return createTaggedError({
    message: `SSB ${source || 'request'} timed out after ${timeoutMs}ms`,
    code: 'SSB_TIMEOUT_ERROR',
    category: 'transport',
    retryable: true,
    details: {
...
async function fetchWithTimeout(
  ...
  // Compose the caller-supplied signal AND the current scan-scope abort so a
  // timed-out pair's in-flight request is actually aborted rather than left
  // running. Never overwrite a caller signal; merge instead.
  const scopeSignal = currentScopeAbortSignal();
  const externalSignal = options && options.signal;
  const signals = [scopeSignal, externalSignal].filter(Boolean);
  ...
  try {
    ...
    if (scopeSignal?.aborted) throw new ScopeAbortError();
...
function classifySSBHttpError({ status, text, source }) {
  ...
  if (status === 401) {
    return createTaggedError({
      message: `SSB ${source} auth failed (${status}): ${normalizedText}`,
      code: 'SSB_AUTH_ERROR',
      category: 'auth',
      status,
      retryable: true
  ...
  if (status === 429 && isHtmlCheckpoint) {
    return createTaggedError({
      message: `SSB ${source} transport checkpoint (${status}): ${normalizedText}`,
      code: 'SSB_TRANSPORT_ERROR',
      category: 'transport',
      status,
      retryable: true
    });
  ...
  if (status === 429 || status >= 500) {
    return createTaggedError({
      message: `SSB ${source} backend failed (${status}): ${normalizedText}`,
      code: 'SSB_BACKEND_ERROR',
      category: 'backend',
      status,
      retryable: true
    });
  ...
  return createTaggedError({
    message: `SSB ${source} request failed (${status}): ${normalizedText}`,
    code: 'SSB_REQUEST_ERROR',
    category: 'validation',
    status,
    retryable: false
  });
...
/**
 * Token manager: in-memory + disk-cached access token with a refresh mutex.
 * Not part of the public createSSBClient options surface — the
 * client factory below is what callers configure.
 *
 * @param {Object} opts - Token manager options.
 * @param {string} opts.authFile - Auth file path used as the token cache namespace.
 * @param {Function} [opts.gotScrapingImpl] - got-scraping implementation for access-token fetch.
 * @param {Function} [opts.now] - Function returning current timestamp in ms. Defaults to Date.now.
 */
function createTokenManager({ authFile, gotScrapingImpl, now }) {
  let cachedToken = null;
  const counters = { tokenRefreshCount: 0, lastTokenRefreshed: null };
  ...
  async function getAccessToken() {
    ...
    tokenRefreshPromise = (async () => {
      try {
        ...
        counters.lastTokenRefreshed = new Date().toISOString();
  ...
  return { getAccessToken, invalidateAccessToken, counters };
}

function makeRequestJSON({ fetchImpl, requestTimeoutMs, getAccessToken, invalidateAccessToken }) {
  async function requestJSON(
    url,
    body,
    { method = 'POST', headers = {}, retryDelays = [], failFast429WithoutRetryAfter = false } = {}
  ) {
    // Cooperative scan-deadline cancellation: refuse to start a NEW request once
    // the owning pair scope has been aborted. This is a fail-closed,
    // non-retryable cancellation (NOT a transport timeout), so the retry loop
    // below must never schedule a follow-up request on an already-cancelled scan.
    if (isScopeAborted()) {
      throw new ScopeAbortError('request not started: scan scope aborted');
    }
    ...
    for (let attempt = 0; attempt < attempts.length; attempt += 1) {
      if (attempt > 0) {
        // Honor a scan-scope abort before sleeping into another retry attempt:
        // once the owning pair is cancelled we must not keep the network busy.
        if (isScopeAborted()) {
          throw new ScopeAbortError('request retry skipped: scan scope aborted');
        }
      ...
      try {
        throwIfScopeAborted();
        const { token } = await waitForScope(getAccessToken());
        ...
        if (!response.ok) {
          ...
          lastError = classifySSBHttpError({ status, text, source: 'HTTP' });
          if (status === 401) {
    ...
    throw lastError || new Error('SSB request failed');
  }

  return requestJSON;
}

function makeTrpcJSONFetcher({ fetchImpl, requestTimeoutMs, retryDelaysMs, getAccessToken, invalidateAccessToken }) {
  async function getTrpcJSON(path, input) {
    ...
    // Circuit breaker keyed by the endpoint PATH (not the full URL with
    // serialized input), so every query variant for the same tRPC procedure
    // shares one breaker — consistent with requestJSON's URL-based keying and
    // so failures across distinct inputs can actually open the circuit.
    const breakerKey = `${TRPC_BASE_URL}/${path}`;
    ...
    for (let attempt = 0; attempt < delays.length; attempt += 1) {
      ...
      if (!response.ok) {
        ...
        const status = Number(response.status || 0);
        const retryable = status === 429 || status >= 500 || status === 401;
        lastError = classifySSBHttpError({ status, text, source: 'TRPC' });
        if (status === 401) {
          invalidateAccessToken();
        }
    ...
    throw lastError || new Error('SSB TRPC request failed');
  }

  return getTrpcJSON;
}

function buildClientMethodsPart0(ctx) {
  const { requestJSON, retryDelaysMs } = ctx;
  return {
...
function buildClientMethodsPart1(ctx) {
  const { requestJSON, retryDelaysMs } = ctx;
  return {
    queryPositiveEV(filters = {}) {
      return requestJSON(
        BACKEND_EV_PROF_URL,
        {
          sportsbooks: ['FanDuel', 'DraftKings', 'BetMGM', 'Caesars', 'Pinnacle', 'NoVigApp'],
          leagues: Array.from(DEFAULT_LEAGUES),
...
function buildClientMethodsPart2(ctx) {
  const { requestJSON, retryDelaysMs } = ctx;
  return {
...
function buildClientMethodsPart3(ctx) {
  const { requestJSON, retryDelaysMs } = ctx;
  return {
...
function buildClientMethodsPart4(ctx) {
  const { requestJSON, retryDelaysMs } = ctx;
  return {
...
function buildClientMethodsPart5(ctx) {
  const { requestJSON, retryDelaysMs } = ctx;
  return {
...
function buildClientMethodsPart6(ctx) {
  const { requestJSON, retryDelaysMs } = ctx;
  return {
...
function buildClientMethodsPart7(ctx) {
  const {
    requestJSON,
    retryDelaysMs,
    now,
    authFile,
    effectiveOddsHistoryMinIntervalMs,
    effectiveOddsHistoryMaxConcurrency
  } = ctx;
  return {
    queryOddsHistory(opts = {}) {
      ...
      throwIfScopeAborted();
      const oddsHistoryInflight = getOddsHistoryInflight();
      ...
      const request = runOddsHistoryRequest(
        ...
        { minIntervalMs: effectiveOddsHistoryMinIntervalMs, maxConcurrency: effectiveOddsHistoryMaxConcurrency }
      )
        .then((result) => {
...
function buildClientMethodsPart8(ctx) {
  const { authFile } = ctx;
  return {
...
function buildClientMethodsPart9(ctx) {
  const { now, authFile, getAccessToken, counters } = ctx;
  return {
    async healthStatus() {
      ...
      return {
        ...
        token: {
          ...
          refreshCount: counters.tokenRefreshCount,
          lastRefreshed: counters.lastTokenRefreshed
        },
        endpoints,
        freshness: {
...
function buildClientMethodsPart10(ctx) {
  const { getTrpcJSON } = ctx;
  return {
    getHiddenBets() {
...
function buildClientMethods(ctx) {
  return Object.assign(
    {},
    buildClientMethodsPart0(ctx),
    buildClientMethodsPart1(ctx),
    buildClientMethodsPart2(ctx),
    buildClientMethodsPart3(ctx),
    buildClientMethodsPart4(ctx),
    buildClientMethodsPart5(ctx),
    buildClientMethodsPart6(ctx),
    buildClientMethodsPart7(ctx),
    buildClientMethodsPart8(ctx),
    buildClientMethodsPart9(ctx),
    buildClientMethodsPart10(ctx)
  );
}

/**
 * Create a SSB API client with built-in auth, retry, and timeout logic.
 * The returned client object includes methods for querying sportsbooks, screen odds,
 * fantasy picks, odds history, and hidden bet management. When a request runs inside a
 * scan cancellation scope (see lib/async-scope.js), the client refuses to start NEW
 * requests after the scope is aborted and composes the scope abort into the live fetch
 * signal — so a timed-out pair stops enqueuing provider work without killing sibling scopes.
 ...
 * @param {number} [options.oddsHistoryMaxConcurrency] - Odds-history gate concurrency (1..8). Defaults to the module default (3). Tests pin serial with 1.
 * @returns {Object} Client object with methods: getAccessToken, querySportsbook, queryPositiveEV, querySmartMoney,
...
function createSSBClient({
  ...
  oddsHistoryMaxConcurrency = ODDS_HISTORY_MAX_CONCURRENCY
} = {}) {
  ...
  const requestedOddsHistoryMaxConcurrency = Number(oddsHistoryMaxConcurrency);
  const effectiveOddsHistoryMaxConcurrency = Number.isFinite(requestedOddsHistoryMaxConcurrency)
    ? Math.max(1, Math.min(8, Math.floor(requestedOddsHistoryMaxConcurrency)))
    : ODDS_HISTORY_MAX_CONCURRENCY;

  const { getAccessToken, invalidateAccessToken, counters } = createTokenManager({ authFile, gotScrapingImpl, now });
  const requestJSON = makeRequestJSON({ fetchImpl, requestTimeoutMs, getAccessToken, invalidateAccessToken });
  const getTrpcJSON = makeTrpcJSONFetcher({
    fetchImpl,
    requestTimeoutMs,
    retryDelaysMs,
    getAccessToken,
  ...
  const client = buildClientMethods(ctx);
  // Preserve the original public surface: getAccessToken was a top-level
  // member of the returned client object (used by tests and callers).
...
module.exports = {
  ...
  SLIPGEN_URL,
  TRPC_BASE_URL,
  // Auth (re-exported from ssb-auth)
  ACCESS_TOKEN_URL,
  DEFAULT_AUTH_FILE,
  DEFAULT_USER_AUTH_FILE,
  REPO_AUTH_FILE,
