...
const { ODDS_HISTORY_REQUEST_BUDGET } = require('./propprofessor-api');
const { getSharpBookComparisonSet, classifySharpBookOrigin } = require('./propprofessor-sharp-books');
const {
  buildUfcShortlist,
...
const { mapWithConcurrency, withPairTimeout } = require('./propprofessor-shared-utils');
...
// (lib/propprofessor-api.js, default 150, env-tunable). Without a
      ...
      const rankedArgs = {
        ...
        ...(quickScreenAggregate
          ? {
              ...
              // process-wide odds-history budget (default 300 calls per
              // 5 min, env PP_ODDS_HISTORY_BUDGET).
              preHistoryGameBudget: Math.max(1, aggregateGameBudget - 1),
              ...
              preHistoryRecoveryGameBudget: Math.max(2, Math.floor(aggregateGameBudget / 4)),
              ...
              preHistoryRecoveryRowBudget: Math.max(2, Math.floor(aggregateGameBudget / 4)),
      ...
      try {
        ...
        const screenCall =
          ...
          String(getLeagueRankingPreset(league).league || league).toUpperCase() === 'TENNIS'
            ...
            : withPairTimeout(
                (args2) => queryLeagueScreen(args2, getLeagueRankingPreset(league).league || league),
                PAIR_TIMEOUT_MS,
...
function buildSharpPlayDiagnostics({ rankedResponses }) {
  const perPairDiagnostics = rankedResponses.map(({ league, market, response }) => ({
    ...
    ...(response?.resultMeta?.classificationSummary
      ? { failureBreakdown: response.resultMeta.classificationSummary.passReasonCounts || {} }
      : {}),
    ...(response?.resultMeta?.emptyState ? { emptyReason: response.resultMeta.emptyState.reason } : {}),
