...
function readLeague(source) {
  const event = source && typeof source.event === 'object' ? source.event : null;
  return (
    readFirst(source, ['league', 'sport', 'leagueName']) ??
    readFirst(event, ['league', 'sport', 'leagueName'])
  );
}
...
function matchSharpOddsEvent(request = {}, candidate = {}, options = {}) {
  ...
  const hasMarketRequest =
    reqMarket.type !== undefined || reqMarket.segment !== undefined || reqMarket.side !== undefined || reqMarket.line !== undefined;
  if (!hasMarketRequest) {
