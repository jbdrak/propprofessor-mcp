...
const RULES = {
  ...
  ncaaf: {
    all: footballRule('NCAAF', { transferContext: true })
  },
  nba: {
...
function footballRule(league, { transferContext = false } = {}) {
  const requiredFields = ['kickoffTimingConfirmed', 'inactiveNewsTimingConfirmed', 'lineIdentityConfirmed'];
  const checks = {
  ...
  // NCAAF-only: transfer/roster effects are position-specific and eligibility/
  // timing dependent, so there is no blanket transfer premium. This flag only
  // records whether upstream transfer context was explicitly confirmed.
  if (transferContext) {
    requiredFields.push('transferContextConfirmed');
    checks.transferContextConfirmed = (value) => value === true;
    reasonCodes.transferContextConfirmed = `${league}_TRANSFER_CONTEXT_MISSING`;
  }
  return { requiredFields, checks, reasonCodes };
}
 ...
 * Only explicit `=== true` flags resolve a gate; nothing is inferred from
 * display strings. The NCAAF `transferContextConfirmed` flag is an
 * evaluation/context status flag (unresolved vs resolved), not an automatic
 * ranker rejection and not a blanket transfer premium.
