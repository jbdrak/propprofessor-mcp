...
// (lib/sharpodds-match.js) behind a single `resolve(row)` seam used only by
// the exact single-game detail path. Broad scans never construct this.
//
// Rules:
...
const { matchSharpOddsEvent, normalizeTeamName } = require('./sharpodds-match');
...
/**
 * @param {{ client?: any, sharpBooks?: string[], timezone?: any }} [options]
 */
function createSharpOddsHistoryProvider(options = {}) {
  const { client, sharpBooks, timezone: defaultTimezone } = options || {};
  const clientValid =
  ...
  function getBoard() {
    if (!boardPromise) boardPromise = client.fetchBoard();
    return boardPromise;
  }
  ...
  async function resolve(row, resolveOptions = {}) {
    ...
    const side = deriveSide(row, tMarket);
    if (!side) return unavailable('side_unknown', 'Could not determine the SharpOdds side (away/home) for this row.');
    ...
    let selectedBook = bookPool.length ? pickSharpBook(bookPool, preferredList) : null;
    if (!selectedBook && !bookPool.length && preferredList.length) {
      selectedBook = { id: null, name: preferredList[0] };
    }
    if (!selectedBook) return unavailable('book_unavailable', 'None of the preferred sharp books post this SharpOdds event.');
