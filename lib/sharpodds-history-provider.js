...
// Mainstream fallback books, in try order. Screen-frontend names only.
// Movement from these books is NEVER sharp-originator evidence: results
// resolved through this list are labeled historyBookTier =
// 'mainstream_fallback' with movementMode = 'comparison_book'.
const DEFAULT_MAINSTREAM_FALLBACK_BOOKS = [
  'FanDuel',
  'Bovada',
  'BetRivers',
  'BallyBet',
...
// True when the matched board event is explicitly marked not covered for
// line history. The current board contract carries no coverage marker, so
// this gate is defensive: any literal 'event_not_covered' coverage/status
// marker (or covered === false) fails closed with zero history fetches.
function isEventNotCovered(rawEvent) {
  if (!rawEvent || typeof rawEvent !== 'object') return false;
  const markers = [
    rawEvent.coverage,
    rawEvent.coverageStatus,
    rawEvent.eventCoverage,
    rawEvent.status,
    rawEvent.eventStatus,
    rawEvent.coverage_state
  ];
  if (markers.some((value) => String(value ?? '').trim().toLowerCase() === 'event_not_covered')) return true;
  return rawEvent.covered === false;
}

function sanitizeBookList(value, fallback) {
  return Array.isArray(value) && value.length
    ? value.map((book) => String(book).trim()).filter(Boolean)
    : [...fallback];
}

// Pick sharp books first, then mainstream fallbacks (excluding books
// already tried in the sharp tier) from one matched event's book pool.
// Preserves the legacy synthetic-book path: with no book pool at all, the
// first preferred sharp book is attempted on its own.
function selectTieredBooks({ bookPool, preferredList, fallbackList }) {
  const selectedSharpBooks = bookPool.length ? pickSharpBooks(bookPool, preferredList) : [];
  const triedNames = new Set(selectedSharpBooks.map((book) => String(book.name).toLowerCase()));
  const selectedFallbackBooks = bookPool.length
    ? pickSharpBooks(bookPool, fallbackList).filter((book) => !triedNames.has(String(book.name).toLowerCase()))
    : [];
  if (!selectedSharpBooks.length && !selectedFallbackBooks.length && !bookPool.length && preferredList.length) {
    return { selectedSharpBooks: [{ id: null, name: preferredList[0] }], selectedFallbackBooks: [] };
  }
  return { selectedSharpBooks, selectedFallbackBooks };
}

// Derive the history-query identity from a matched board event.
function describeMatchedEvent(matched, { homeTeam, awayTeam, league }) {
  ...
  return {
    ...
    eventHome: rawEvent?.homeTeam ?? rawEvent?.home_team ?? rawEvent?.home ?? rawEvent?.hn ?? homeTeam,
    eventAway: rawEvent?.awayTeam ?? rawEvent?.away_team ?? rawEvent?.away ?? rawEvent?.an ?? awayTeam
  };
}

// Find the first board entry matching the PP row identity (teams +
// league + start time). Returns { matched, sawTeamOverlap } where matched
// is the match wrapper or null. sawTeamOverlap is true when any candidate
// agreed on teams ('teams_match') even if league/time later rejected it,
// letting resolve() distinguish absent-team coverage (event_not_covered)
// from same-team mismatch (event_mismatch).
function findMatchedBoardEvent(entries, request, resolveOptions) {
  const matchOptions =
  ...
  for (const { event: rawEvent, groupLeague } of entries) {
    ...
    if (Array.isArray(result?.reasons) && result.reasons.includes('teams_match')) sawTeamOverlap = true;
    if (result && result.matched) {
      return { matched: { rawEvent: candidate, groupLeague, match: result }, sawTeamOverlap: true };
    }
  }
  return { matched: null, sawTeamOverlap };
}

// Sharp-first, mainstream-fallback history resolution across one matched
// event. Sharp books are tried first with unchanged same_book provenance;
// mainstream fallbacks are tried only when no sharp book produced usable
// history, and usable fallback history is relabeled comparison_book with
// historyBookTier = 'mainstream_fallback' so it can never read as
// sharp-originator evidence. Identity/side/line gates live inside
// fetchHistoryEntry and apply equally to both tiers.
async function resolveTieredHistory({ selectedSharpBooks, selectedFallbackBooks, historyArgs }) {
  let sharpResult = null;
  if (selectedSharpBooks.length) {
    sharpResult = await resolveSelectedBookHistory({ ...historyArgs, selectedBooks: selectedSharpBooks });
    if (sharpResult?.lineHistoryAvailable === true) return sharpResult;
  }
  if (!selectedFallbackBooks.length) {
    return sharpResult || unavailable('history_unusable', 'No preferred SharpOdds book returned usable history.');
  }
  const fallbackResult = await resolveSelectedBookHistory({ ...historyArgs, selectedBooks: selectedFallbackBooks });
  if (fallbackResult?.lineHistoryAvailable === true) {
    return { ...fallbackResult, movementMode: 'comparison_book', historyBookTier: 'mainstream_fallback' };
  }
...
/**
 * @param {{ client?: any, sharpBooks?: string[], fallbackBooks?: string[], timezone?: any, boardTtlMs?: number, historyCacheMaxEntries?: number }} [options]
 */
function createSharpOddsHistoryProvider(options = {}) {
  const { client, sharpBooks, fallbackBooks, timezone: defaultTimezone, boardTtlMs, historyCacheMaxEntries } = options || {};
  const clientValid =
  ...
  const defaultSharpBooks =
    Array.isArray(sharpBooks) && sharpBooks.length
      ...
      : [...DEFAULT_SHARP_BOOKS];
  const defaultFallbackBooks = sanitizeBookList(fallbackBooks, DEFAULT_MAINSTREAM_FALLBACK_BOOKS);
  ...
  async function resolve(row, resolveOptions = {}) {
    ...
    const preferredList =
      Array.isArray(resolveOptions.sharpBooks) && resolveOptions.sharpBooks.length
        ...
        : defaultSharpBooks;
    const fallbackList = sanitizeBookList(resolveOptions.fallbackBooks, defaultFallbackBooks);
    ...
    const boardMatch = findMatchedBoardEvent(
      entries,
      { homeTeam, awayTeam, league, startTime: startMs },
      resolveOptions
    );
    const matched = boardMatch?.matched ?? null;
    // No team overlap on the board means the game is not covered here —
    // not a near-miss on time/league. Same teams with a rejected
    // league/time is an event_mismatch.
    if (!matched) {
      if (!boardMatch?.sawTeamOverlap) {
        return unavailable('event_not_covered', 'SharpOdds event is not covered for line history.');
      }
      return unavailable('event_mismatch', 'No SharpOdds event matched this game (teams/league/time).');
    }

    // Never fabricate history for an uncovered event: no book is attempted.
    if (isEventNotCovered(matched.rawEvent)) {
      return unavailable('event_not_covered', 'SharpOdds event is not covered for line history.');
    }

    const eventBooks = extractBooks(matched.rawEvent);
    const bookPool = rootBooks.length ? rootBooks : eventBooks;
    const { selectedSharpBooks, selectedFallbackBooks } = selectTieredBooks({ bookPool, preferredList, fallbackList });
    if (!selectedSharpBooks.length && !selectedFallbackBooks.length) {
      return unavailable('book_unavailable', 'None of the preferred sharp books post this SharpOdds event.');
    }

    const { eventId, historyLeague, eventHome, eventAway } = describeMatchedEvent(matched, {
      homeTeam,
      awayTeam,
      league
    });
    ...
    return resolveTieredHistory({
      selectedSharpBooks,
      selectedFallbackBooks,
      historyArgs: {
        historyCache,
        maxHistoryCacheEntries,
        client,
  ...
  return { resolve, DEFAULT_SHARP_BOOKS, DEFAULT_MAINSTREAM_FALLBACK_BOOKS };
}
