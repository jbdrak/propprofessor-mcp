...
const { normalizeSharpOddsHistory, parseTimezoneOffsetMinutes } = require('./sharpodds-history');
...
const NON_FULLGAME_RE =
  /(1st|2nd|3rd|4th|first|second|third|fourth|half|quarter|period|inning|live|1h|2h|1q|2q|3q|4q|1p|2p|3p|\bq[1-4]\b|\bp[1-3]\b)/i;
...
function deriveSide(row, tMarket) {
  ...
  const candidates = [
    ...
    String(row?.selectionId ?? row?.selection_id ?? '')
      .split(':')
      .pop()
  ];
...
function deriveRequestedLine(row, tMarket) {
  const explicit = row?.line ?? row?.requestedLine ?? row?.requested_line;
  const explicitLine = explicit === null || explicit === undefined || explicit === '' ? null : Number(explicit);
  if (Number.isFinite(explicitLine)) return explicitLine;

  const pickText = String(row?.pick ?? row?.selection ?? row?.participant ?? '').trim();
  if (tMarket === 'TOTAL') {
    const match = pickText.match(/^(?:over|under)\s+(\d+(?:\.\d+)?)$/i);
    return match ? Number(match[1]) : null;
  }
  const match = pickText.match(/\s([+-]\d+(?:\.\d+)?)$/);
  return match ? Number(match[1]) : null;
}
...
function historyIdentityMatches(normalized, { homeTeam, awayTeam, league, dateKey, selectedBook }) {
  ...
  if (event.sportsbook && String(event.sportsbook).trim().toLowerCase() !== String(selectedBook).trim().toLowerCase())
    return false;
...
async function fetchHistoryEntry({
  ...
  try {
    ...
    return unavailable(
      'history_parse_error',
      `SharpOdds history could not be parsed (${(error && error.message) || 'parse error'}).`
    );
  }
  ...
  if (tMarket !== 'ML') {
    const linePoints = normalized.points.filter((point) => Number.isFinite(Number(point.line)));
    if (linePoints.length !== normalized.points.length) {
      return unavailable('history_unusable', 'SharpOdds line history contains missing historical line values.');
    }
    if (
      Number.isFinite(Number(requestedLine)) &&
      !linePoints.some((point) => Math.abs(Number(point.line) - Number(requestedLine)) <= 1e-9)
    ) {
      return unavailable(
        'line_mismatch',
        `SharpOdds history has no point for the requested line ${Number(requestedLine)}.`
      );
    }
  }
  if (
...
// Scan the fetched board for the requested event. Tracks whether any
// candidate shared both participants so the caller can distinguish a
// coverage gap (event_not_covered: e.g. WTA 125/Challenger absent from the
// board) from a genuine same-teams league/time conflict (event_mismatch).
// Time tolerance is never widened here.
function findBoardMatch(entries, request, matchOptions) {
  let matched = null;
  ...
  for (const { event: rawEvent, groupLeague } of entries) {
    const candidate =
      ...
      (rawEvent.league === undefined || rawEvent.league === null) &&
      groupLeague
...
function createSharpOddsHistoryProvider(options = {}) {
  ...
  const clientValid = client && typeof client.fetchBoard === 'function' && typeof client.fetchHistory === 'function';
  ...
  async function resolve(row, resolveOptions = {}) {
    ...
    if (!tMarket)
      return unavailable(
        'market_unsupported',
        `SharpOdds lookup does not support market "${row?.market ?? row?.playType ?? 'unknown'}".`
      );
    ...
    if (!requestedSide)
      return unavailable('side_unknown', 'Could not determine the SharpOdds side (away/home) for this row.');
    ...
    if (!Number.isFinite(startMs))
      return unavailable('start_time_missing', 'SharpOdds lookup needs a parseable start time.');
    const league = readLeague(row);
    const requestedLine = deriveRequestedLine(row, tMarket);
    ...
    const matchOptions =
      Number.isFinite(Number(resolveOptions.timeToleranceMs)) && Number(resolveOptions.timeToleranceMs) > 0
        ...
        : undefined;
    const { matched, sawTeamOverlap } = findBoardMatch(entries, request, matchOptions);
    if (!matched && !sawTeamOverlap) {
      return unavailable('event_not_covered', 'SharpOdds board has no event for these teams (event not covered).');
    }
    ...
    const eventId =
      rawEvent?.event && typeof rawEvent.event === 'object'
        ? (rawEvent.event.id ??
          rawEvent.event.eventId ??
          rawEvent.event.gameId ??
          rawEvent.id ??
          rawEvent.eventId ??
          rawEvent.gameId ??
          rawEvent.n)
    ...
    const historyLeague =
      rawEvent?.league ??
      rawEvent?.sport ??
      (rawEvent?.event && rawEvent.event.league) ??
      matched.groupLeague ??
      league ??
      null;
    ...
    return resolveSelectedBookHistory({
      ...
      side:
        tMarket === 'TOTAL'
          ? requestedSide
          : matched.match.order === 'swapped'
            ? flipHistorySide(requestedSide)
            : requestedSide,
      offsetParam,
      dateKey,
      eventId,
