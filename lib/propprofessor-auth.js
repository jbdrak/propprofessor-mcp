...
const DEFAULT_EGO_TASK_SPACE = 7;
...
/**
 * Resolve the ego task space id to reuse: explicit option wins, then
 * `PROPPROFESSOR_EGO_TASK_SPACE`, then the default (7).
 * @param {number|string|undefined} explicit - Explicit task space id.
 * @returns {number} Positive integer task space id.
 */
 ...
 * @param {number} taskSpaceId - ego task space id to reuse.
...
function buildEgoScript(taskSpaceId, accessTokenUrl) {
  ...
  return [
    'try {',
    `  await useOrCreateTaskSpace(${taskIdLiteral});`,
 ...
 * (default 7) and writes exactly one JSON line via `process.stdout.write`
 * (ego-browser routes script output to stderr; the Node side scans the
 * combined capture for that one contract line — arbitrary diagnostics are
 * never treated as token data). Set `PP_NO_EGO_FALLBACK=1` to disable this
 * fallback.
 *
 * @param {Object} [options] - Options object.
 * @param {number|string} [options.taskSpaceId] - ego task space id to reuse. Defaults to env `PROPPROFESSOR_EGO_TASK_SPACE` or 7.
 ...
 * the browser already solved the TLS-fingerprint challenge. If Chrome is
 * not reachable either, falls back to the logged-in ego-browser task
 * space, which inherits the user's PropProfessor login.
 *
...
/**
 * Try the browser fallbacks in order — CDP first, ego-browser second —
