    ...
    for (const group of groups.values()) {
      ...
      for (const { stance, resolved } of group.stances) {
        ...
        if (matched && typeof exactFn === 'function') {
          // Once an exact recheck is available, the broad rank row is only a
          // discovery candidate. Never fall back to its stale/degraded labels
          // when the authoritative per-game lookup fails or cannot reconcile
