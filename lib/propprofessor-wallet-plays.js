...
function leagueFromSlug(slug) {
  const prefix = String(slug || '').split('-')[0].trim().toLowerCase();
...
function verdictForRow(row) {
  const r = row || {};
  const mov = String(r.movementDisposition || '').trim().toLowerCase();
  const tier = String(r.confidenceTier || '').trim().toUpperCase();
  const clvRaw = r.recentClvPct;
  const clv =
    typeof clvRaw === 'number' ? clvRaw : String(clvRaw ?? '').trim() === '' ? NaN : Number(clvRaw);
