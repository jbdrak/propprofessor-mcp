 ...
 * @returns {Promise<{ wallets: Array<{ wallet, stances }>, droppedByPrefix: Array<{prefix, count, example}>, nonSportsDropped: number }>}
 */
async function analyzeWalletPlays(opts) {
  ...
  if (!Array.isArray(wallets) || wallets.length === 0) return { wallets: [], droppedByPrefix: [], nonSportsDropped: 0 };
  const walletRows = await polyWallets.fetchWalletStancesAll(wallets, o);
  if (!Array.isArray(walletRows) || walletRows.length === 0) return { wallets: [], droppedByPrefix: [], nonSportsDropped: 0 };
  ...
  const NON_SPORTS_PREFIXES = new Set([
    'what', 'fed', 'bitcoin', 'highest', 'lowest', 'elon',
    'kazakhstan', 'south', 'spider', '2026', '2nd', 'largest',
    'fl', 'us', 'republican', 'iran', 'strait', 'the', 'presidential',
    'who', 'best', '1', 'bra'
  ]);
  ...
  for (const row of walletRows) {
    ...
    for (const stance of row.stances) {
      ...
      if (!resolved) {
        ...
        const prefix = String(stance.eventSlug || '').split('-')[0].trim().toLowerCase();
        if (prefix) {
