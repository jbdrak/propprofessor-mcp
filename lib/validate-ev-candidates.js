...
const { mapWithConcurrency, createCrossCallMemoizedQuery } = require('./ssb-shared-utils');
const { getSharpBookComparisonSet } = require('./ssb-sharp-books');
const { buildPositiveEvTarget } = require('../scripts/server/handlers/handler-utils');
const { resolveHistoryForEntity } = require('./ssb-history');
...
async function validatePositiveEvCandidate({
  client,
  play,
  args,
  requestedBooks,
...
async function validatePositiveEvCandidates({ client, candidates = [], args = {} } = {}) {
  ...
  const enriched = await mapWithConcurrency(rows, async (play) => {
    const enrichedRow = await validatePositiveEvCandidate({
      client,
      play,
      args,
      requestedBooks,
    ...
    if (enrichedRow.validationFailed) {
