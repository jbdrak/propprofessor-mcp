...
async function resolveScreenHistoryRow(
  ...
  if (Array.isArray(row.lineHistory) && row.lineHistory.length >= 2) {
    return {
      ...
      historyBookTier: row.historyBookTier || null
    };
  }
  ...
  if (resolved?.lineHistoryAvailable) {
    return {
      ...
      historyBookTier: resolved.historyBookTier || null,
  ...
  return {
    ...
    ...(resolved && typeof resolved === 'object' && 'historyErrorStatus' in resolved
      ...
      : {}),
    ...(resolved && typeof resolved === 'object' && 'historyBookTier' in resolved
      ? { historyBookTier: resolved.historyBookTier || null }
      : {})
  };
}
...
async function resolveHistoryWithLineFallback({
  ...
  if (preferSharpOddsHistory && (sharpResult || sharpError)) {
    return {
      ...
      ...(sharpResult?.historyBookTier ? { historyBookTier: sharpResult.historyBookTier } : {}),
      ...(sharpError ? { historyProvider: 'sharpodds', historyReason: 'provider_error', historyWarning: sharpError?.message || String(sharpError) } : {})
    };
  }
