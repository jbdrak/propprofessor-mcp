...
async function getSelectionMovement(client, gameId, selectionId) {
  let clv = null;
  ...
  if (!selectionId || !gameId) return { clv, movementDisposition, bookCount, clvSource, openingOdds, currentOdds };
  try {
    ...
    for (const histBook of HISTORY_BOOKS) {
      ...
      if (!Array.isArray(bookHistory) || bookHistory.length < 2) continue;
  ...
  return { clv, movementDisposition, bookCount, clvSource, openingOdds, currentOdds };
}
 ...
 * @param {string|string[]} [opts.markets] - Explicit market(s) to query; defaults to the standard Tennis markets
 * (Moneyline, Game Handicap, Total Games). Set Handicap is explicit-only.
 ...
 */
async function recoverTennisFromScreen({ client, book, markets: requestedMarkets, skipTimeCorrection = false }) {
  ...
  const markets = [
    ...new Set(
      (explicitMarkets.length ? explicitMarkets : ['Moneyline', 'Game Handicap', 'Total Games']).flatMap((market) =>
  ...
  for (const market of markets) {
    ...
    for (const game of games) {
      ...
      for (const sel of Object.values(game.selections)) {
        ...
        if (odds1 && selection1) {
          const movement = await getSelectionMovement(client, game.gameId, sel.selection1Id);
          plays.push(
            createFallbackPlay({
              game,
              market,
        ...
        if (odds2 && selection2) {
          const movement = await getSelectionMovement(client, game.gameId, sel.selection2Id);
          plays.push(
            createFallbackPlay({
              game,
              market,
