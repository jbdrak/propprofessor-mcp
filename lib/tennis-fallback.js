 ...
 * inside the shared 75-call window (which also serves the caller's own
 * hydration), and the effective budget is clamped to what actually remains
 * in the window at call time — never hundreds of blind attempts.
 */
const DEFAULT_MAX_HISTORY_SELECTIONS = 60;

// Headroom kept under the shared 75-call window when the fallback sizes
...
async function collectTennisFallbackCandidates({ client, book, markets }) {
