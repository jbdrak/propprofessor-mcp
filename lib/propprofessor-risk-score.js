...
function gradeMovementQuality(item = {}) {
  ...
  const movementGradeRaw = String(item.movementGrade || '').trim().toLowerCase();
  const movementDisposition = String(item.movementDisposition || '').trim().toLowerCase();
  ...
  if (!movementLabel) {
    if (movementGradeRaw === 'green') movementLabel = 'supportive';
      ...
      const mov = String(item.movement || '').trim().toLowerCase();
...
function calculateRiskScore(item = {}) {
  ...
  if (lastMoveAgeMs > 0) {
    ...
    if (lastMoveAgeHours < 1) score -= 1;        // moved < 1h ago — strong recency
    else if (lastMoveAgeHours < 3) score -= 0.5;  // moved < 3h ago — moderate recency
