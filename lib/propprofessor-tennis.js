...
function flashscoreTimeToISO(timeStr, dateStr) {
  ...
  // Build a Date in CDT (UTC-5 during CDT, UTC-6 during CST)
  // We use a fixed offset since tennis matches are always during CDT (Mar-Nov)
  const dt = new Date(`${dateStr}T${hours.padStart(2, '0')}:${minutes}:00-05:00`);
...
async function correctTennisTimes(rows) {
    ...
    if (match && match.time) {
      if (isPlaceholderTime(espnMatches, match.time, match.status)) {
        ...
        if (diffMs > 30 * 60 * 1000 || !startTime) {
          ...
          // Time already correct — still mark as ESPN-verified
          row.startSource = 'espn-verified';
          row.startMatchName = match.match;
          row.startConfidence = Math.round(match.confidence * 100) / 100;
          if (match.venue) row.startVenue = match.venue;
          if (match.status) row.startStatus = match.status;
  ...
  // Web search fallback for rows ESPN didn't cover (process in parallel with concurrency)
