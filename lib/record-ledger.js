...
function defaultLedgerPath() {
  return process.env.PP_RECORD_LEDGER || pathModule.join(os.homedir(), '.ssb-for-agents', 'tracker', 'ledger.json');
}
...
function loadLedger(opts = {}) {
  ...
  if (!integrity.ok) return { ok: false, error: `Invalid ledger integrity: ${integrity.errors.join('; ')}` };
