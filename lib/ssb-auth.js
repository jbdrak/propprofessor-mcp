...
// The upstream app moved from app.propprofessor.com to www.propprofessor.com
// (verified 2026-09-11): app. now 30x-redirects there. Both hosts are accepted
// when locating a logged-in tab, so the browser fallbacks survive a move back.
...
// configurable via `SSB_CDP_VERSION_URL` so this Mac can point at
...
// SSB login, so a same-origin browserFetch sails past Vercel's
 ...
 * Falls back to SSB_COOKIES env var if no auth file is found.
...
function readAuthState(authFile = resolveAuthFile()) {
  ...
  try {
    ...
    const cookiesEnv = String(process.env.SSB_COOKIES || '').trim();
    if (cookiesEnv) {
      try {
    ...
    const message =
      error?.code === 'ENOENT'
        ? `SSB auth file not found: ${authFile}. Set AUTH_FILE env var or SSB_COOKIES for cookie-based auth.`
        : error?.code === 'EACCES'
          ? `SSB auth file not readable: ${authFile}`
          : `Failed to read SSB auth file at ${authFile}: ${error?.message || error}`;
...
function isSSBDomain(domain) {
  const normalized = normalizeDomain(domain);
  return normalized === 'propprofessor.com' || normalized.endsWith('.propprofessor.com');
}

/**
 * Check whether an auth object is valid (has at least one non-empty SSB cookie).
 * @param {*} auth - The auth object to validate.
 * @returns {boolean} True if the auth object has valid SSB cookies.
 */
...
function getCookieExpiryInfo(auth, nowFn = Date.now) {
  ...
  const ppCookies = auth.cookies.filter(
    (c) => c && isSSBDomain(c.domain) && typeof c.expires === 'number' && c.expires > 0
  );
  ...
  if (!sessionCookie) {
    ...
    const sessionCookies = auth.cookies.filter(
      (c) => c && isSSBDomain(c.domain) && c.name && c.name.includes('session')
    );
...
/**
 * Build a Cookie header string from auth state, filtering only SSB domain cookies.
 ...
 */
function buildSSBCookieHeader(authState) {
  const cookies = Array.isArray(authState?.cookies) ? authState.cookies : [];
  return cookies
    .filter((cookie) => cookie && isSSBDomain(cookie.domain))
...
function inspectAuthSetup() {
  ...
  const checkedPaths = getAuthFileCandidates().map((file) => {
    ...
    try {
      ...
      if (exists) {
        ...
        parseable = true;
        const cookies = Array.isArray(authState?.cookies) ? authState.cookies : [];
        ssbCookieCount = cookies.filter((cookie) => cookie && isSSBDomain(cookie.domain)).length;
      }
    } catch (cause) {
  ...
  return {
    ok: selectedEntry.exists && selectedEntry.readable && selectedEntry.parseable && selectedEntry.ssbCookieCount > 0,
    source,
    selectedAuthFile,
 ...
 * `SSB_CDP_VERSION_URL`, then the default (127.0.0.1:9222).
...
function resolveCdpVersionUrl(explicit) {
  ...
  const envRaw = String(process.env.SSB_CDP_VERSION_URL || '').trim();
...
async function waitForCdpAppOrigin(send, sid, deadlineMs) {
  ...
  for (;;) {
    if (Date.now() >= deadline) {
      ...
      throw new Error(`CDP: created tab did not settle on the SSB app origin within ${deadlineMs}ms (${seen})`);
    }
    ...
    if (
      ...
      APP_HOST_ALIASES.some((host) => lastState.href.includes(host)) &&
...
async function evaluateTokenFetch(send, sid) {
  // Fetch a SAME-ORIGIN relative path. The tab may land on www or app depending
  // on the upstream redirect, and an absolute cross-origin URL throws
  // "Failed to fetch" even when the cookies are valid.
  const urlLiteral = JSON.stringify(new URL(ACCESS_TOKEN_URL).pathname);
...
/**
 * Fetch a SSB access token via Chrome DevTools Protocol from a
 ...
 * override with the `versionUrl` option or `SSB_CDP_VERSION_URL`).
 * Returns `{ token, exp, perm }` on success; throws on any failure.
 *
 * @param {Object} [options] - Options object.
 * @param {string} [options.versionUrl] - Chrome DevTools version endpoint. Defaults to env `SSB_CDP_VERSION_URL` or `http://127.0.0.1:9222/json/version`.
...
async function fetchAccessTokenViaCDP({
  ...
  try {
    ...
    // 3. Find or create a tab on the SSB app origin (www. or app.).
    const appOriginUrl = new URL(ACCESS_TOKEN_URL).origin + '/';
    const targets = await send('Target.getTargets');
    let tab = (targets.targetInfos || []).find(
      (t) => t.type === 'page' && APP_HOST_ALIASES.some((host) => (t.url || '').includes(host))
    );
    let tid = tab && tab.targetId;
    let createdTab = false;
    if (!tid) {
      const created = await send('Target.createTarget', { url: appOriginUrl });
 ...
 * `SSB_EGO_TASK_SPACE`, then the default named space.
...
function resolveEgoTaskSpaceId(explicit) {
  ...
  const envRaw = String(process.env.SSB_EGO_TASK_SPACE || '').trim();
  if (envRaw) {
    const n = Number(envRaw);
    if (Number.isInteger(n) && n > 0) return n;
    throw new Error(`Invalid SSB_EGO_TASK_SPACE value: ${envRaw}`);
  }
...
function buildEgoScript(taskSpaceId, accessTokenUrl) {
  ...
  // Open the app ORIGIN and fetch a same-origin relative path: the tab may land
  // on www or app after the upstream redirect, and an absolute cross-origin URL
  // makes browserFetch throw "Failed to fetch" even when the cookies are valid.
  const appOriginLiteral = JSON.stringify(new URL(accessTokenUrl).origin + '/');
  const pathLiteral = JSON.stringify(new URL(accessTokenUrl).pathname);
  return [
    'let task;',
    'try {',
    `  task = await useOrCreateTaskSpace(${taskIdLiteral});`,
    // Ensure a same-origin tab exists before browserFetch: a task space with
    // zero tabs has no well-defined page context to fetch from.
    `  await openOrReuseTab(${appOriginLiteral}, { wait: true, timeout: 20 });`,
    `  const raw = await browserFetch(${pathLiteral}, {`,
    "    method: 'GET',",
    "    credentials: 'include',",
    "    headers: { Accept: 'application/json' }",
    '  });',
...
/**
 * Fetch a SSB access token through the logged-in ego-browser task
 * space. ego-browser's isolated task-space browser inherits the user's
 * SSB login, so a same-origin `browserFetch` to the access-token
 ...
 * script reuses the task space id from `SSB_EGO_TASK_SPACE`
 ...
 *
 * @param {Object} [options] - Options object.
 * @param {number|string} [options.taskSpaceId] - ego task space id to reuse. Defaults to env `SSB_EGO_TASK_SPACE` or the named default space `pp-token-refresh`.
...
async function fetchAccessTokenViaEgo({
  ...
  return await new Promise((resolve, reject) => {
    ...
    try {
      ...
      reject(new Error(`Failed to spawn ego-browser: ${spawnError?.message || String(spawnError)}`));
      return;
    }
    ...
    child.on('error', (spawnError) => {
      finish(() => reject(new Error(`Failed to start ego-browser: ${spawnError?.message || String(spawnError)}`)));
    });
...
/**
 * Fetch a SSB access token using stored auth cookies.
 *
 ...
 * space, which inherits the user's SSB login and sails past
 ...
 * @throws {Error} If no SSB cookies are found or all token refresh paths fail.
 */
async function fetchAccessToken({
  ...
  const cookieHeader = buildSSBCookieHeader(authState);
  if (!cookieHeader) {
    throw new Error(`No SSB cookies found in ${authFile}`);
  }
  ...
  if (statusCode !== 200 || !data || !data.token) {
    ...
    throw new Error(`Failed to fetch SSB access token: ${message}`);
  }
