...
process.on('unhandledRejection', (reason) => {
  ...
  process.stderr.write(`[ssb-for-agents] Unhandled Rejection: ${redactSecrets(raw)}\n`);
});
process.on('uncaughtException', (error) => {
  const raw = error?.stack || error;
  process.stderr.write(`[ssb-for-agents] Uncaught Exception: ${redactSecrets(raw)}\n`);
});

/**
 * SSB MCP server entry point.
 *
...
// setting SSB_MCP_MODE=full.
const VALID_MODES = new Set(['full', 'lite']);
const TOOL_MODE = (() => {
  const raw = (process.env.SSB_MCP_MODE || '').toLowerCase().trim();
...
function createMcpServer({
  ...
  rateLimiter = new RateLimiter({
    maxCalls: Number(process.env.SSB_RATE_LIMIT || 25),
    windowMs: Number(process.env.SSB_RATE_WINDOW_MS || 60000)
  })
} = {}) {
    ...
    if (method === 'tools/call') {
      ...
      try {
        ...
        process.stderr.write(
          `[ssb-for-agents] tool=${toolName} code=${categorized.code} category=${categorized.category} message=${safeMessage}\n` +
            (safeStack ? `${safeStack}\n` : '')
        );
...
function claimPidFile() {
  if (process.env.SSB_MCP_NDJSON === 'true') {
  ...
  const PID_FILE = '/tmp/ssb-for-agents.pid';
  try {
...
async function serveStdio(options = {}) {
  ...
  const client = options.client || createSSBClient();
  const handlers = createMcpHandlers({ client });
  const server = createMcpServer({ handlers, toolDefinitions: buildToolDefinitions({ mode: TOOL_MODE }) });

  // Opt-in write coalescing: controlled by SSB_MCP_STDIO_COALESCE_MS env var.
  // Default 0 = no coalescing (current behavior preserved).
  const coalesceMs = Number(process.env.SSB_MCP_STDIO_COALESCE_MS || 0);
  ...
  // SSB requests at startup — it only talks to the backend in
...
if (require.main === module) {
  ...
  const args = ['--mcp'];
  if (process.env.SSB_MCP_MODE) args.push('--mode', process.env.SSB_MCP_MODE);
