...
const { analyzePlayerPropBet } = require('../lib/ssb-analysis');
...
const { buildRankedScreenResponse, getDebugFlag } = require('../lib/ssb-mcp-ranked-screen');
const { createMcpHandlers } = require('./ssb-mcp-server');
...
function getCommandInventory() {
  return [
    ...
    {
      command: 'login',
      description: 'Open a browser to log in to SSB and save auth automatically (requires playwright)'
    },
    {
      command: 'init',
      description:
...
function buildDoctorReport(healthResult) {
  ...
  if (!node.ok) {
    ...
    nextStep = `Save your SSB browser session to ${auth.defaultUserAuthFile} or set AUTH_FILE, then rerun \`pp-query doctor\`.`;
...
async function runInitCommand({ opts, client, logger }) {
  ...
  if (!authInfo.ok) {
    logger.log('→ No valid auth found. Opening browser for SSB login...');
    try {
  ...
  logger.log('');
  const scriptPath = path.resolve(__dirname, 'ssb-mcp-server.js');
  const config = {
    mcpServers: {
      ssb: {
        command: 'node',
...
async function main({ argv = process.argv, client = createSSBClient(), logger = console } = {}) {
